...
**⑦ Standing open items (unchanged):** the 7/7 51-fill $279k reverse-check gap; structural ledger-blindness to remediation + close_long flows (fold into pre-GO reconciliation). Local clone is **~20 commits behind `origin/master`** (harmless — trading runs in GitHub Actions, not locally; sync via `git pull` any time). rclone Drive backup completion unconfirmed since the 7/20 both-directions timeout (`--ignore-existing` makes a timeout safe; escalate only if it persists). The 5/12 first-ever v25.1 run log (`25732008349`) expires ~Aug 10 if anyone wants it archived for the stale-era record.
