...
| **NEXT TRADING RUN** | **Verify the `pnl_history` ET fix `c225537` rebuilt the series correctly (9th dated change — MEASUREMENT, not model behaviour)** | 🔴 **Merged but NOT yet verified against a live rebuild.** The file is rebuilt from Alpaca every run, so it self-heals on the next snapshot — no backfill. **Falsifiable prediction: Mon 0→10, Sat 8→0, Fri 10→8, still 48 rows, and `2026-08-04` present** (the newest completed session stopped being deleted). Fri drops to 8 because 6/19 and 7/3 were market holidays — exactly why only 8 phantom Saturdays existed. **Anything other than zero Saturdays and ten Mondays means the fix is wrong — escalate before the next Stage-1 read.** Then **set `QT_PNLDATE_FIX_FROM`** in `validate_gross_cap.yml` to flip section 12's live-file check from informational to a hard regression gate. ⚠️ **Post-fix WRC and GPR reads WILL shift — that is the re-dating, NOT drift:** GPR buckets by month, so 7/31's session moves out of August back into July. Full detail: 8/05 ledger ⑧. |
...
**🔴 VERIFICATION STILL PENDING at time of writing.** No trading run had executed post-merge. **Falsifiable prediction for the next run's rebuild — check it:**

| | pre-fix (48 rows) | predicted post-fix |
|---|---:|---:|
| Mon | **0** | **10** |
| Tue / Wed / Thu | 10 / 10 / 10 | 10 / 10 / 10 |
| Fri | 10 | **8** |
| Sat | **8** | **0** |
| newest completed session | absent (deleted each run) | `2026-08-04` present |

Fri drops to 8 because 6/19 and 7/3 were holidays — which is precisely why only 8 phantom Saturdays ever existed. **If the rebuild returns anything other than zero Saturdays and ten Mondays, the fix is wrong — escalate before the next Stage-1 read.**
...
- **Verify the pnl_history rebuild on the next run** (⑧), then **set `QT_PNLDATE_FIX_FROM`** in `validate_gross_cap.yml` to turn section 12's live-file check into a hard regression gate.
- **Frame-1 provisional-row restatement (⑦) is documented, NOT fixed.** Decide whether the analyzer should drop the unsettled row.
- **`beta_roll` verdict deferred ~3 sessions** (⑤) — leaning warm-up.
