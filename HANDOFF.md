...
**⏭️ Proof due the 8/07 morning run — prediction PRE-REGISTERED in the checkpoint row above, written before the run fired.** Headline: expect `withholding 2026-07-31`, `days (N): 13` over `2026-07-14 -> 2026-07-30`, and the `2026-07-30` row to **change from −0.0155** to its settled value. **The proof is not that value — it is that on Mon 8/10 nothing moves again.**
...
- 🔴 **GitHub runner-acquisition failures (④) — now FOUR tonight and they BLOCKED a merge.** ⑥'s fix is written, self-reviewed and documented but sits **unmerged** on `fix/settled-rank-ic-rows` (`5b20739`) because `validate` section 14 could never execute (`31127868282` cancelled, `31128040795` failed at 15m1s, both "job was not acquired"). **Do not merge it unvalidated** — the branch moves `validate_gross_cap.py`'s summary block, and a wrong edit there makes every future validate run a false pass. **If the 8/07 13:35Z morning dispatch also fails to acquire a runner, that is a real evidence gap across all three clocks** — and Frame-3 is ~4 obs from its ≥30 decision-grade read.
- **The 8/07 prediction is VOID if the fix did not merge first** — re-baseline procedure written into that checkpoint row (user-approved 8/06). Part (d), stability across sessions, survives any re-baseline and is the test that actually matters.
...
**NOT a dated model change** — no signal, order, sizing or gate is altered. **The legacy series is deliberately NOT retired:** it is what the Stage-1 gate has always read, and switching outright would restart the decision window a third time with no overlap to compare against. v2 needs ~30 obs → decision-grade **~mid-September**. `validate ALL PASS 31050474120` (sections 1–12 unregressed + new **section 13, 31 checks**, incl. a behavioural replay proving `rank_score` survives the very flatten that destroys `confidence`, and that it carries names below 0.5 where the legacy column never does); `preflight 31050472195`. **Verification landed 8/06 — see that ledger.**
