...
| 🛑 **~2026-09-25** (was ~9/24) | **STOP CRITERION S4 — Frame 1 v2 at 30 obs; the only genuinely unmeasured thing left** | **RETIRE Frame 1 if it misses the Stage-1 gate** (rank-IC ≥ +0.03 AND t ≥ 2.0). ⚠️ **Frame 1 has used BOTH its window restarts** (`b2a15f5` 7/14, v2 8/06) — **this read is FINAL whatever is discovered afterward.** 📅 **DATE CORRECTED 2026-08-27.** Rows land on a **measured 6-trading-session settle lag** (run 8/24→pred 8/14, 8/25→8/17, 8/27→8/19), and **Labor Day 9/07 removes a pred date**, so obs #30 is pred **2026-09-17**, written by the **9/25** run. Under the old 9/25 sunset the series would have stopped at **N=29 — one short of the pre-registered 30**; sunset moved to 9/26 to close that gap. This does NOT change the verdict (t=0.32 vs a bar of 2.0) — it removes a technicality someone could later argue from. See 8/27 ledger ①. |
...
| ⏳ **Sat 2026-09-26 — CRONS SELF-RETIRE** (was Fri 9/25) | **The wind-down completes itself; NO human action required** | `sunset` gate job in `quant_daily.yml` (`f919412`) skips the `trade` job on/after `QT_SUNSET_DATE='2026-09-26'`. 🔑 **THREE files carry this date and ALL must stay matched** — `quant_daily.yml`, `morning_watchdog.yml` **and `universe_watch.yml`**. ⚠️ The third was missed by the "**in both files**" instruction this row used to carry; an unmatched copy pages/posts to Discord every weekday forever over an expected absence. **Last run day is Fri 2026-09-25.** Expect `trade` to report **SKIPPED, not failed** — that is success. ⚠️ Verify the last read (S4, ~9/25) landed BEFORE this fires. Afterwards `gh workflow disable quant_daily.yml morning_watchdog.yml universe_watch.yml` is optional cosmetic cleanup. |
