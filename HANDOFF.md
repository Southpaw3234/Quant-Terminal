...
> ### 🔴 TWO BLOCKERS OPEN — HON RAISED 2026-08-12 (STILL UNCLEARED), THE BROKEN RETRAIN RAISED 2026-08-22. "nothing needs you today" IS THE WRONG ANSWER UNTIL BOTH CLEAR.
>
...
> `QT_MAX_GROSS='0'` blocks. **54 red trading runs across 10 days** (6 on 8/12,
> then ~5/day 8/13–8/21 — latest `32529839370`, 8/21 21:43Z), zero successes.
> ⚠️ **And that permanent red is what hid blocker 2 for a full day** — see the 8/22 ledger ④.
> **TWO open items — do them before anything else:**
>
...
> 2. 🔴 **VERIFY THE RETRAIN CAME BACK — Mon 2026-08-24.** The 8/21 morning run
>    trained **`0/0` models** (river 0.26 broke Cell 2 → `_get_macro_history` →
>    `build_features`); pin `01b2897` (`river>=0.21,<0.26`) is pushed but has NOT
>    yet run a morning. **Full diagnosis: 8/22 ledger ①-⑤.** Check, in order:
>    (a) any weekend cycle installs `river-0.25.0` and prints `River online
>    learning imported OK` with no `Cell 2 raised` / `Cell 6 raised`;
>    (b) Monday's morning log shows **`~304/304 trained`** and a **~2h cycle,
>    not 4 minutes**; (c) the auto-commit touches `data/shadow/` again
>    (`cross_sectional_positions.jsonl` gets an 8/24 book — it stops at 8/20);
>    (d) `data/predictions/dsr_scores.json` is repopulated from `{}`.
>    ⚠️ **If (b) fails, do not let it ride** — every broken morning is a lost
>    Frame-1 day inside the S4 window (terminal read ~9/24).
> 3. ✅ **DONE — the invariant fix is MERGED** (`f46eef9`, PR #23; post-merge
