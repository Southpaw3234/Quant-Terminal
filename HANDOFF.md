...
🔍 **UNRESOLVED, and recorded as unresolved rather than guessed:** `pos_map` read **24** on every 8/10 run and **25** on 8/11, with no intervening fill. If HON sat at the broker throughout, 8/10 should also have read 25. Not explicable from the available artifacts. It does not affect the wind-down (the book is flat and confirmed twice), but it means the broker position count has been observed to move without a trade — worth one look if it recurs.
