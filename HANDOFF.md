...
> `QT_MAX_GROSS='0'` blocks. Five consecutive runs red on 8/12. **Two open items,
> neither done — do these before anything else:**
>
> 1. **Cover HON.** `gh workflow run position_trim.yml --repo Southpaw3234/Quant-Terminal -f mode=execute -f sleeve=short-cover`
...
> 2. **Merge or close `fix/flat-invariant-short-split` @ `64e5677`** (`validate ALL
>    PASS 31649513045`, 303 checks, 0 fail). Pushed, no PR. It does NOT fix HON —
>    it stops a stray short pinning the gate red and hiding a real re-entry.
>
...
**⑤ ✅ FIX DRAFTED, VALIDATED, PUSHED — AND DELIBERATELY NOT MERGED.** Branch `fix/flat-invariant-short-split`, commit `64e5677`. Longs and shorts become separate states with separate remedies:
...
**⑥ ⏭️ TWO THINGS ARE STILL OPEN — NEITHER IS DONE.**

1. 🔴 **HON IS NOT COVERED.** Master's crons keep failing and paging until it is. The fix branch does **not** address this — covering is what reaches flat, and the invariant only goes green on the first run *after* the fill:
   ```
...
2. 🔴 **`fix/flat-invariant-short-split` IS NOT MERGED.** No PR opened. Master is untouched.
