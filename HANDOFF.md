...
- [ ] **Evidence engines still recording?** THREE clocks now: Frame 1 — `data/shadow/rank_ic.csv` + `cross_sectional_ls.csv` (incl. `ls_hedged`); Frame 3 — `data/stat_arb/stat_arb_ls.csv`; Frame 2 (from 7/10) — `data/shadow_intraday/predictions.csv` + `rank_ic.csv` + `cross_sectional_ls.csv`. All gaining rows on morning runs. If any is empty/stale → the evidence clock stalled, fix immediately (see CHECKPOINT section).
- [ ] **AUC attribution (from 7/10):** the tz fix `c1bf94e` put 5 real intraday columns into the live feature set on 7/10 — a dated model change. When reading walk-forward AUC/IC, compare against the pre-change reference **0.5516 / 0.0834 (7/9)** and attribute shifts to the fix, not to alpha appearing.
...
## 🗓️ SESSION LEDGER — 2026-07-11 (Saturday): weekend-scoring cron MISFIRED as a full morning retrain + live BUY — run-type ordering bug (4th dup-retrain-class incident), FIXED `66a860d`
...
| Zero orders | ❌ **BUY GE: conformal-discounted 25 → 10 (×0.44), ~$3,770, queued for Mon 7/13 open** (within the 1-slot room; user may cancel in Alpaca before Monday, else it fills) |
...
**② Root cause + FIX `66a860d` (workflow-only, NOT a model change):** weekend days (Sat AND
