...
**③ Watch item — walk-forward collapsed:** 12-fold mean OOS **AUC 0.4973 / IC −0.0130**
("weak/no edge") vs 0.5437/0.0700 (7/13) and 0.5516/0.0834 (7/9). Attributed to the 5th dated
change for now, BUT the fix was supposed to be training-bit-identical (all training paths do
their own `dropna(subset=["target"])`) — a drop this size is either fold noise, one new
trading day shifting fold boundaries, or the fix changed walk-forward panel composition.
Scrutinize on 7/15: if it stays sub-0.50, diff the walk-forward panel row counts pre/post
`5e96366` before reading anything into it. ADWIN also flagged drift same run.

**④ Handed to Wednesday 7/15:** walk-forward re-read (③); confirm the analyzer prints the
restart-exclusion line on the 7/15 morning run and exits clean; fill_audit ~7/17+; Sat 7/18
cron proof; attn_vol20 trainer-exclusion decision still open.
