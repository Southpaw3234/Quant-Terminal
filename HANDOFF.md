...
**⑩ 🔴 THE CROSS-SECTION SILENTLY VARIES IN SIZE, AND IT REACHES INTO THE S4 WINDOW.** Chasing ⑦'s "305 not 307" turned up a **fifth silent-failure of the familiar shape**. The universe is not fixed at 307 — it has been drifting all month, and nothing records it:
...
⏭️ **Logged, NOT actioned — deliberately.** The obvious fix is one print statement (make `SKIP` state its reason and row count, matching `DELISTED?`), and a universe-composition line per run. That is a genuine harness improvement and squarely inside "keep the harness" — **but it touches the live notebook, there is no local Python to check it with, and every change in this repo goes branch → `validate` → PR.** Doing it mid-session against a live concurrency group would be exactly the shortcut this project has been burned by. **Next session: fix the logging, then look at why `AVB`/`EQR` specifically keep missing the batch.**
