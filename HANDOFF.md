...
| **Thu 2026-07-16 ~12:15 ET** | **Short-cover + oversell-guard verify** — auto-verify task `qt-thu-0716-cover-verify` ARMED **9:31 AM ET** (fires before the 9:35 morning dispatch grabs the concurrency group; runs only while the Claude app is open — catches up on next launch; covers check (a) + handoff bookkeeping, the morning-log checks (b)-(e) remain for the daily pickup ~12:15) | (a) 22 BUY-to-cover fills landed at the open (run `29447931911`) → Alpaca shows ZERO short positions, gross ~0.15-0.25× (0.15× + any morning entries), composition `us_equity/long` only — check via a position_trim dry-run read or the morning gate line; (b) morning log prints `[patch] Oversell guard: enforce=True pos_map=~58+ symbols pos_ok=True`; (c) any `[oversell]` BLOCKED/capped lines are EXPECTED (ledger still overstates holdings — each line is a divergence surfacing, log them, don't panic); (d) close_long prints no short-deepening behavior; (e) with ~8 BUY slots open expect the first multi-entry morning since 7/8 — entries are conformal-sized, gross-capped, and now oversell-guarded. If any short position EXISTS after the open → the guard missed a path, escalate before the next cycle. |
