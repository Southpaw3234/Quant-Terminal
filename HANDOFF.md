...
| **Exits** | ✅ **Still run.** The cap is BUY-only (`quant_runner.py:5723`); SELLs use `_oversell_cap` (:5726). 🔴 **BOOK IS NOT FLAT.** HON x13 short ≈$2,714. The 09-01 "flat" reading was FALSE — Alpaca returned an empty position list and every guard believed it; HON has been short continuously since before 08-11 and no cover was ever placed. See the 09-04 ledger ①. |
| **Crons** | ⏳ **SELF-RETIRING 2026-09-29** — no human action required. A `sunset` gate job in `quant_daily.yml` computes the ET date; on/after `QT_SUNSET_DATE` the `trade` job is **skipped** (not failed, so it raises no alarms). Costs ~5s/tick instead of ~2h. Until then they keep firing so the final reads land. |
...
S1 (Frame 3, ~Wed 8/12), S2 (Frame 2, ~8/21), S4 (Frame 1 v2, ~9/28) already
...
> ### 🔴 ONE BLOCKER OPEN — HON, RAISED 2026-08-12, STILL UNCLEARED. "nothing needs you today" IS THE WRONG ANSWER UNTIL IT CLEARS.
>
...
> `QT_MAX_GROSS='0'` blocks. **77 failures / 33 cancelled since 8/12, zero
> successes** on `quant_daily.yml` (latest `32997555725`, 8/26 18:03Z).
> ⚠️ **That permanent red is what hid BOTH silent failures** — the 8/21 broken
> retrain (8/22 ledger ④) and the stale-base evidence wipe (8/25 ledger ⑥).
> 🔑 **Both are now fixed AND proven, and the lesson is structural: any new alarm
> must be placed where HON's red cannot swallow it.** The `Predictions ledger
...
>    ✅ **DRY-RUN RE-CONFIRMED 2026-08-26 03:49Z (`32927896291`, nothing submitted) — the plan NAMES HON.**
>    `HON short=-13 openBUY=0 price=216.00 cover=13 cost=$2,808 P&L -1,174` → *"1 market
>    BUY-to-cover (DAY, fill at next open) … Projected after covers: gross ~$0
>    (0.00x equity), zero shorts."* Account `equity=$113,781.55 cash=$116,589.55`,
>    **1 position — HON is now the ENTIRE book** (gross $2,808, 0.02×). This clears all three hazards: it is **not
...
>    ⚠️ The printed `P&L −1,174` is the **distorted-basis artifact — do NOT book it.**
...
> 2. ✅ **CLOSED — THE RETRAIN CAME BACK** (verified 8/25, holding since). Pin
>    `01b2897` (`river>=0.21,<0.26`) proven over three mornings: 8/24 trained
>    `306` models, 8/25 `307`, 8/26 `307` (`1 skipped`), and
>    `dsr_scores.json` is repopulated (**5821 bytes at `data/predictions/`** —
>    note the path, it is *not* directly under `data/` and mis-grepping it reads
>    as still-gutted). **Full diagnosis: 8/22 ledger ①-⑤.**
> 3. ✅ **CLOSED — THE STALE-BASE EVIDENCE WIPE IS FIXED AND PROVEN** (`86947a6`,
>    PR #25; proof landed 8/26). The ledger guard reported `OK — ledger did not
>    shrink (51008 -> 51313)` and held flat across four later runs, and the exact
>    8/24 race recurred without biting: `32982441632`, **created 14:47Z inside the
>    morning cycle**, read the live tip. **Full diagnosis: 8/25 ledger ②-⑥; proof:
>    8/26 ledger ①.** ⏭️ Nothing to do here — just don't re-open it on a grep hit.
> 4. ✅ **DONE — the invariant fix is MERGED** (`f46eef9`, PR #23; post-merge
...
> The gate signature is byte-identical on every run sampled 8/17–8/21:
...
| 🛑 **~2026-09-28** (was ~9/25) | **STOP CRITERION S4 — Frame 1 v2 at 30 obs; the only genuinely unmeasured thing left** | **RETIRE Frame 1 if it misses the Stage-1 gate** (rank-IC ≥ +0.03 AND t ≥ 2.0). ⚠️ **Frame 1 has used BOTH its window restarts** (`b2a15f5` 7/14, v2 8/06) — **this read is FINAL whatever is discovered afterward.** 📅 **DATE CORRECTED TWICE — 2026-08-27, then 2026-09-01.** Rows land on a **measured 6-MARKET-SESSION settle lag** (run 8/24→pred 8/14, 8/25→8/17, 8/27→8/19, 8/28→8/20, 9/01→8/24). ⚠️ **The lag counts SESSIONS, but the obs count only advances on days that HAVE predictions — and 2026-08-21 has NONE** (the unpinned `river` 0.26 break that aborted Cell 2, fixed by `01b2897`; see 8/22 ledger). The 8/27 correction counted 8/21 as an observation and was therefore one session short. **Confirmed live 9/01:** the run wrote pred **8/24** and skipped 8/21 outright rather than backfilling it, so the hole is permanent. With 8/21 excluded and **Labor Day 9/07** removed, obs #29 is pred **2026-09-17** (written by the 9/25 run) and obs #30 is pred **2026-09-18**, written by the **9/28** run. Sunset moved **9/26 → 9/29** (PR #28) so that run happens. Does NOT change the verdict — N=12 reads **+0.0128, t=0.37** against a bar of t≥2.0 — it removes a technicality someone could later argue from. See 8/27 ledger ① and 9/01. |
...
| ⏳ **Tue 2026-09-29 — CRONS SELF-RETIRE** (was Sat 9/26) | **The wind-down completes itself; NO human action required** | `sunset` gate job in `quant_daily.yml` (`f919412`) skips the `trade` job on/after `QT_SUNSET_DATE='2026-09-29'`. 🔑 **THREE files carry this date and ALL must stay matched** — `quant_daily.yml`, `morning_watchdog.yml` **and `universe_watch.yml`**. ⚠️ The third was missed by the "**in both files**" instruction this row used to carry; an unmatched copy pages/posts to Discord every weekday forever over an expected absence. **Last run day is Mon 2026-09-28.** Expect `trade` to report **SKIPPED, not failed** — that is success. ⚠️ Verify the last read (S4, ~9/28) landed BEFORE this fires. Afterwards `gh workflow disable quant_daily.yml morning_watchdog.yml universe_watch.yml` is optional cosmetic cleanup. |
...
## 🗓️ SESSION LEDGER — 2026-09-04 (Friday, session 2): ✅ **BOTH SPEC-#2 AVAILABILITY CHECKS RAN AND BOTH CLEAR — Finnhub returns three full years (561 events, 278 tickers), and dedup at 63 bars leaves 127 settled against a bar of 40.** 🔀 **THE TRADING/INVESTING FORK IS SETTLED: long-horizon systematic, horizon 63 bars**, chosen by measurement (252 bars = zero settled observations). 🔴 **SURVIVORSHIP BIAS INSIDE THE ENGINE FOUND AND FIXED** — a name that stopped trading mid-hold was silently dropped as "unmatured"; and the probe proved the universe half is NOT fixable for free (0/3 bankruptcies serve price history). ⚠️ **S4 keeps climbing: N=15, +0.0398, t +1.28.** K still 1/5. Nothing re-read.

**① THE FORK — SETTLED, AND v27 HAD BEEN ON THE WRONG SIDE OF ITS OWN LINE.** `NEXT_ARCHITECTURE.md` §② defines investing as a *years* horizon. Spec #1 used **21 trading days**. The project had been described as investing while being measured as trading. Decision: **long-horizon systematic** — same machinery, horizon out of the days-scale cell; autonomous execution retained; §③'s instrument panel deferred, not refuted. `docs/V27_FORK_DECISION.md` (PR #47).

**Horizon chosen by arithmetic, not preference.** Against the frozen 205-event set: 21d → 193 settle, **63d → 170**, **252d → 0**. A one-year horizon has zero observations on one year of history. **Horizon = 63 bars** (`event_study.py` default, PR #46). 🔑 For a real effect, mean scales ~linearly with horizon while sd scales with √horizon, so `t` improves with √horizon — this is the one lever that attacks E1's actual failure (`sd=19.22%` swamping `+1.76%`) rather than just buying sample.

**② 🔴 SURVIVORSHIP BIAS INSIDE THE ENGINE — found, fixed, and honestly scoped.** `event_study._fwd_ret` returned a bare `None` whenever `entry_i + horizon` ran off the end of the series. Correct for an event too recent to mature; **catastrophic for a name that stopped trading, because both look identical.** Delisted names — overwhelmingly losses — were silently dropped in the same bucket as a healthy three-week-old event. Survivorship *inside the measurement*, on top of the one in the universe, and it scales with horizon.

Fixed: `qt.prices.forward_return_ex` returns `(ret, status)` with `ok | unmatured | delisted`, classified structurally ("now" = freshest bar in the data, not the wall clock). ⚠️ **A delisting return is the move to the LAST TRADED PRICE, not −100%** — acquisitions delist too and are gains; assigning a wipeout swaps one bias for a bigger one. `qt.measurement.survivorship_sensitivity` reports every read three ways (all / survivors / delisted) so a number cannot be quoted without showing how far it moves. Test: dropping 6/60 delistings turned **−6.20% into +2.00%** — what the old engine did silently. (PR #46.)

⚠️ **Honest scope: this fixes the EDGE of the problem, not the bulk.** It catches names still in the symbol list whose series ends. Names that delisted *before* the list was read never enter the pipeline at all — no event is dropped, because none was ever created. Check 2 below confirms it: `delisted = 0` on the current set, **because every ticker in it is a survivor by construction.** That zero is the bias, not its absence.

**③ THE PROBE — there is NO free fix for the universe half, and it is now demonstrated rather than asserted.** `probe_delisted.py` (PR #48) asked whether yfinance serves known-delisted tickers: **BBBYQ, SIVBQ, WEWKQ (Ch.11) → none. ATVI, VMW (acquired) → none. FRCB → data, but FRCB is still in Finnhub's symbol list and never tested the question.** `0/3 bankruptcies`. The dead names have no prices to fetch, so a historical ticker list would not help even if one existed. **Recovering them is a paid source — Sharadar or Polygon, ~$50–100/mo — or the result stays a ceiling with an unknown floor.**

⚠️ **My own probe printed the wrong verdict first.** Its pass condition was `if hits:` — "at least one" — and on 1/6 it announced "✅ the price side works." A probe whose pass condition is "at least one" cannot fail, and I wrote it to check a claim I had made. Rebarred to bankruptcies specifically. **Sixth misleading green this session, and the first I authored.**

**④ THE PRICE LAYER (`qt/prices.py`, PR #45) — and the back-adjustment claim made checkable.** The folklore "back-adjusted series leak" is half right: **returns are INVARIANT** (the factor cancels in the ratio, so A2 is correct on adjusted data and raw prices would be *worse*); **dollar ADV is ~invariant** (price × volume survives a split); **the price floor is NOT** — after a 1-for-10 reverse split a stock that traded at $0.50 reads $5.00 and clears a $2 floor it should fail. Reverse splits signal distress, so back-adjustment systematically **admits** what the floor excludes. Also exposed: `build_universe.py:472` calls `compute_adv(asof=None)` — point-in-time eligibility was built in A4 and never wired. `screen_as_of()` fixes it and returns a *reason*, not a boolean.

**⑤ THE AGENT AND THE MODEL ARE MERGED (PR #44).** `event_study.py` now delegates to `qt.ledger` and `qt.measurement`; the Phase-1 reproduction gate proved the swap moved nothing (`+1.7647%, t +1.16, stability −0.52`, exact). **The Referee is load-bearing:** `_referee_gate()` runs before any price is fetched, and the E1 job sets `QT_SPEC_ID=form4_cluster_buy_v1`, so **that job can no longer re-run the spent read** — refused with exit 3. The K budget stopped being something remembered and became something enforced.

**⑥ ✅ CHECK 1 — DOES FINNHUB RETURN 1,095 DAYS? YES, FULLY.** Run `33927199851`, 907 tickers, lookback 1,095d:

| | 365d (spec #1) | 1,095d |
|---|---|---|
| raw insider tx | 38,261 | **110,454** |
| code P | 3,527 | **11,443** |
| cluster-buy events | 205 | **561** |
| distinct tickers | 125 | **278** |

Filings per year: 2023 (≈3.5 mo) 7,936 · **2024 35,442 · 2025 37,276** · 2026 (8 mo) 29,793. Two full years at ~36k each and the partial years on pace — **the tier does not cap.** (Six rows dated 2014 and one 2022 are mis-dated artifacts, not a window.) Events: 51 / 171 / 211 / 128 by year. Committed as `data/events/events_form4_1095d.csv` — **NOT** over `events_form4.csv`, which is spec #1's frozen input. The 10MB raw file is reproducible via dispatch and not committed.

**⑦ ✅ CHECK 2 — DEDUP AT 63 BARS: 127 SETTLE, CLEARS N≥40 BY 87.** Run `33927201211`, `QT_EVENT_COUNT_ONLY=1` (new — dedup + settlement classification via index arithmetic only, no price divided by another, Referee bypassed because nothing to authorise, job asserts nothing written): 205 → **−51 overlapping** (was −33 at 21 bars) → 154 independent → **127 ok, 27 unmatured, 0 delisted, 0 no_price.** Sample size is not the constraint at a quarter horizon. See ② for why `0 delisted` is the bias confirming itself.

**⑧ WHAT THE CHECKS MEAN FOR SPEC #2.** Both preconditions in `V27_FORK_DECISION.md` §⑥ clear. Rough power on the 3-year set at 63 bars: 561 → ~420 after dedup (~25% loss observed) → ~390 settled — above the proposal's ~330 estimate. **Every projection still rides on the assumption that the effect accrues linearly with horizon.** If spec #1's `+1.76%` was a right-skewed noise draw, tripling the horizon multiplies noise, not signal, and `t` does not move. Spec #2 (`horizon 63 + lookback 1,095`) was **DECLARED 2026-09-04 as `form4_cluster_buy_v2`** (PR #51) after both checks cleared — **K still 1/5**, a declaration spends nothing. Frozen: input `events_form4_1095d.csv`, output `event_study_v2.csv` (the E1 job refuses to write any other spec into v1's frozen `event_study.csv`, exit 4). 🔴 **The §⑤ prior is still REQUIRED BEFORE THE READ** and is blank; the Referee authorises without it, the documents do not.

⚠️ **Survivorship is materially WORSE on the 3-year set.** Three years of small-cap delistings are missing instead of one, all of them invisible, all of them biasing upward. If spec #2 is read, its result is a **ceiling**, must be reported with the caveat beside the number, and the sensitivity report will show `delisted = 0` for the same by-construction reason.

**⑨ ⚠️ S4 — N=15, mean +0.0398, t +1.28** (09-01: N=12, t 0.37 · 09-04 a.m.: N=14, t 0.98). Rows 8/24–8/27 all positive. The remaining 15 sessions need roughly **+0.048/day against a window mean of +0.040 — about 1.2×**, down from 5× a week ago. Still a miss; four positives on sd≈0.12 is unremarkable; WRC/SPA untouched. But it has moved from implausible to genuinely uncertain, and **a pass on 09-28 reopens Frame 1** under the pre-registered rule. Not to be dismissed.

**⑩ HOUSEKEEPING.** The e1 job had `QT_EVENT_HORIZON: '21'` hardcoded — stale since the default moved to 63; any future dispatch would have silently run at the retired horizon. Un-pinned (PR #49). `v27/**` validation workflows now also fire on `v28/**` after the merge commit changed `event_study.py` on a v28 branch and nothing ran. Book still short HON ×13, gate red, evidence still advancing on red runs. Nine PRs merged this session (#43–#49 plus the merge conflicts resolved by hand and verified against the reproduction gate rather than trusted). **Live pipeline untouched; no-touch rule holds to 09-29.**

**⑪ ⚠️ SPEC #2 MERGED WITH ITS OWN TEST UNRUN — the seventh stale trigger.** `v27/declare-spec2` changed the registry and `validate_qt_phase1.py`, both in `validate_qt.yml`'s paths filter — but that workflow fired on `v28/**` only, so the hand-written registry JSON and `test_spec2_declared` reached master **without the Referee ever loading them.** The merge chain's CI gate was designed to require two workflows and fell through on timeout to a check that asked only whether *whatever ran* was green. Caught immediately after: a manual dispatch on master passed all ten spec-2 checks (registry loads, v2 declared/unread/authorisable as "2/5", K 1/5), so the content was sound and the gap was in verification. `validate_qt.yml` now fires on `v27/**` too (PR #52). The v27 workflows had been widened to `v28/**` earlier for the mirror failure; the reverse was never done. **A trigger condition is part of the test and goes stale like any other — and a gate that checks "was what ran green" instead of "did the expected things run" is not a gate.**

**⑫ ✅ WRC/SPA BUILT — the E1 clause that had never been evaluated is now enforced (PR #53).** Until today, "must clear WRC and SPA at p<0.10 against K=5" was a NOTE printed under every read; a pass could not have been called on the day it happened. `qt/wrc.py` implements White (2000) and Hansen (2005) over a Politis–Romano stationary bootstrap. **Two layers, and the distinction is load-bearing:** the joint tests need K *aligned* series, which event studies do not have — spec #1 and #2 fall on different names and dates — so `event_study_correction` bootstraps each spec's event-level returns and applies a Bonferroni bound at **α/K_declared = 0.02, charging the unread slots.** Correcting only for specs actually tested would let the budget be spent one read at a time, each judged as if it were the only one. **The load-bearing test is the data-snooping demonstration, measured not asserted:** best-of-5 pure noise → naive one-sided t rejects **38%** at α=10% (theory ≈41%); **WRC 8%, SPA 9%.** A t≈4.9 signal among four nulls still reads p=0.0000. K > K_declared RAISES. The E1 report now prints the correction after the point estimate and says a point-estimate PASS is not an E1 pass unless that line also reads PASS. Registry caveat updated to match (PR #54). **Consequence for spec #2: the effective bar is raw bootstrap p ≤ 0.02, not 0.10.**

**⑬ 🔴 A MISWIRED DISPATCH READ THE WRONG SPECIFICATION — recorded, fixed, guarded, and put to the operator.** Run `33933992017` was authorised by the Referee for `form4_cluster_buy_v2` and wrote to the v2 ledger correctly, but the engine LOADED `events_form4.csv` — v1's 205-event, 365-day file — not the declared 561-event, 1,095-day file. Cause: the e1 job resolved `QT_EVENTS_CSV` as `inputs.events_csv || <1095d default>`, and `events_csv` is a dispatch input shared with the count job whose declared default is the v1 file — non-empty on every dispatch, so the fallback could never fire. **Same class as `if typ and typ not in …`: a fallback that cannot trigger.** Caught because "dropped 51 / wrote 127 / 27 unmatured" matched check 2 on the v1 set to the digit; confirmed by the ledger's date span (2025-09 → 2026-06, the v1 window). What was seen — an **undeclared** 63-bar × 365-day configuration: `n=127, +4.00%, t 1.08, stability 0.29, WRC-Bonf p 0.125` → NOT MET. `record_read` was never called; `read_at` stayed empty. Preserved as `event_study_v2_MISWIRED_365d.csv`. **Fix:** e1 gets its own `e1_events_csv` input, and an **INPUT GUARD** reads the registry's declared `events_file` for the spec and refuses (exit 5) anything else — the Referee already knew the right file; the job never asked it. **Proven by negative test** (run `33934415838`): replaying the exact miswire stops before a price downloads. **Both options — forfeit or proceed — were put to the operator.** Recorded in the registry as a deviation either way. **Operator decision: PROCEED**, on the basis that the declared inputs were never opened, the parameters were frozen before the peek, and the peek was a NOT MET. Committed before dispatch (20:55:35 ET).

**⑭ 🔴 SPEC #2 READ ON ITS DECLARED INPUTS — NOT MET. K IS 2/5.** Run `33934605296`, dispatched 00:56:33Z with `input guard OK`, prior 10% recorded 16 seconds before the first dispatch attempt.

```
561 events → −128 overlapping (63-bar window) → 433 → 406 settled · 27 unmatured · 0 delisted
n=406   mean +1.19%   t +0.62   stability UNDEFINED (first third −0.20% → last +2.48%)   40% positive
WRC-Bonf raw p 0.2595   bar 0.02 (3 unread slots charged)
E1: NOT MET   — cleared N and mean; missed t, stability, WRC
```
...
On 2026-09-01 Alpaca's position endpoint returned an **empty list**. The account read itself SUCCEEDED — equity came back as $116,590 — so `quant_runner.py:3499` ran:
...
**An empty list iterates zero times and sets `pos_ok = True`.** The system recorded *"successfully read zero positions"* and has no way to distinguish that from *"the account is flat."*

Everything downstream agreed: `gross $0 (0.00x)`, `pos_map=0 symbols`, `book=0 longs=0 shorts=0`, `flat: true`, and the gate printed `OK — armed and flat (book=0)`. **Twice, nine hours apart** (runs `33514260085`, `33548106772`).

✅ **IT WAS FALSE, AND THIS IS ESTABLISHED, NOT INFERRED.** HON x13 was short throughout and reappeared in the 09-02 read. **No cover order was ever placed:** `position_trim.yml` has never run in this repository's history, and the operator confirms placing nothing. The position did not come back — **it never left.**

🔑 **Why this outranks one red gate:**
* **`close_long` walks the HELD BOOK.** Against an empty read it closes nothing and reports `closed 0 | errors 0` — a clean success. A wind-down would have silently done nothing while looking healthy. **That is the 2026-08-10 `close_long` failure arriving through a different door.**
* **The flat invariant — the guard whose entire job is detecting a non-empty book — was SATISFIED by a broker glitch.**
* Any execution layer built on "the broker is the source of truth" inherits this exactly.

**FIXED IN v28, NOT IN v25.** `qt.guards.position_read_trustworthy()` encodes the invariant: **a book cannot go from non-empty to flat without a fill.** An empty read is self-evidencing only when the prior book was already empty, or when a fill explains it. `fills_since=None` FAILS CLOSED. Seven tests including a regression named for line 3499.

⚠️ **`quant_runner.py` was deliberately NOT patched** — the no-touch constraint runs to 09-29, the gate is currently RED *correctly*, entries are off, and the crons retire in 25 days. Low live risk; the design lesson is the durable part.

⚠️ **RETRACTION.** The 09-01 ledger claimed HON was covered and cited `short_acked: []` as proof. That reasoning was sound about **acknowledgement** — it correctly ruled out a human silencing the gate via `QT_FLAT_ACK_SHORT` — and **worthless about truth.** It checked whether the flat state had been faked by a person and never asked whether the broker read underneath it was real. **HON has been short continuously since before 08-11.**

**② S4 HAS MOVED, AND THE OUTLOOK CHANGED.** N=14, window 08-06 → 08-26, mean **+0.0315**, **t = +0.98**. The last three rows landed +0.0455, +0.181, +0.1062.

| read | N | mean | t | remaining run-rate needed |
|---|---|---|---|---|
| 09-01 | 12 | +0.0128 | 0.37 | +0.064/day (~5× the window mean) |
| **09-04** | **14** | **+0.0315** | **+0.98** | **+0.055/day (~1.7×)** |

⚠️ **Do not over-read this.** t=0.98 against a bar of 2.0 is still a clear miss, three consecutive positives on sd≈0.12 is unremarkable, and the WRC/SPA gate is untouched. But the honest statement changed: at N=12 a pass was arithmetically implausible; at N=14 it is merely unlikely. **If S4 passes on 09-28 it reopens Frame 1**, and that possibility should be held in mind rather than dismissed.

✅ Clocks healthy: `rank_ic_v2` last written 09-03 16:06, `predictions.csv` complete through 09-03 with no holes. **Evidence still advances on RED runs** — the state commit precedes the flat gate, proven by N=13→14 landing while every run was failing.

**③ v28 PHASES 0 AND 1 — the `qt` package.** Replacement for `quant_runner.py` (404KB) and `trading_model_v25.1.ipynb` (516KB). Not a rewrite of the model, which is measured and retired, but of the machinery.

* **Phase 0** — `qt/ledger.py` (ONE frozen-ledger implementation replacing TWO that had already drifted apart) and `qt/guards.py` (gross cap, sector cap, oversell, drawdown, all fail-closed, each naming its incident). ⚠️ **One flagged behavioural change: v25's sector cap FAILS OPEN**; the port defaults to fail-closed with `fail_closed=False` reproducing v25.
* **Phase 1** — `qt/measurement.py` (the only role permitted to compute a return) and `qt/referee.py` (owns the pre-registration, can VETO). **Exit gate PASSES: Measurement reproduces the frozen E1 read exactly** — `n=160, mean +1.7647%, t +1.16, stability −0.52, first +1.0061% → last −0.5196%`.
* **`data/registry/specifications.json`** is live: `K 1/5 spent, 4 remaining; terminal 2027-03-31`, with the survivorship / next-session-entry / WRC caveats stored **with** the number.

**Neither phase spent K.** Building machinery is free; only a read costs.

**④ ⚠️ FOUR GREEN SIGNALS POINTED AT THE WRONG THING — the reusable lesson.** None was found by reading code; all four by asking whether a green tick meant what it appeared to mean.

1. A freeze test passing **vacuously** — it perturbed prices by a constant ×1.25, but a return is a ratio, so the factor cancelled exactly and the test asserted only that identical inputs give identical outputs.
2. A pipeline guard **crying wolf** — `HEAD~1` does not exist under `actions/checkout`'s default depth 1, so `git diff` errored and the `!` inverted the error into a violation report on every file.
3. **Preflight passing without compiling a line of the new code** — check 1 covered nine v25 files and none of the v27/v28 components. Now 15 checks, covering all of them.
4. **A `paths:` filter that lagged its own tests** — the Phase 1 exit gate sat un-run while the branch's last green tick referred to an earlier commit.

**⑤ NAMING — recorded because it is already confusing.** There is no v26. **v27 and v28 are PARALLEL TRACKS, not sequential versions:** v27 is the *alpha search* (pre-registration, K=5, event-study edge layer); v28 is the *machinery rebuild* (agent team, `qt/` package). The numbering wrongly implies one supersedes the other.

**⑥ RECOMMENDED REORDER: do Phase 3 (Risk + Execution) BEFORE Phase 2 (Research proposes spec #2).** Phase 3 costs nothing and Phase 2 spends one of four remaining specifications; the false-flat incident just supplied Phase 3's requirement in detail, with a real failure to test against; and spec #2 has no deadline before E1's 2026-12-15.

**⑦ OPEN.** HON short 13 shares ≈ $2,714, gate red. It is a trade the operator fires (`position_trim.yml`, `sleeve=short-cover`) — and ⚠️ **the fill must be verified directly, because a subsequent flat reading is exactly the signal that lied on 09-01.** Covering does not protect S4; evidence advances regardless. It restores the ability to *see* a new failure.

## 🗓️ SESSION LEDGER — 2026-09-01 (Tuesday): ⚠️ **ITEM ① OF THIS ENTRY IS RETRACTED — see the 09-04 ledger ①. HON WAS NEVER COVERED; the "flat" reading was a false broker read and the position was short throughout. The rest of this entry stands.** ~~🎉 HON IS COVERED — THE BOOK IS FLAT AND THE 21-DAY RED STREAK IS OVER.~~ ✅ **S4's read date was off by ANOTHER trading day** — 2026-08-21 produced zero predictions (the `river` break), the 8/27 correction counted it as an observation, and the sunset moved 9/26 → 9/29 so obs #30 actually lands (PR #28). 🆕 **v27 OPENED AND SCOPED**: build plan + a SIGNED pre-registration (P=15%, K=5, $10,000), then four edge-layer components built and merged in one session — event-study engine, Form 4 extractor, inverted universe screen, full-population pricing. 🔑 **Three separate live probes each contradicted code that had passed its own tests**, which is the reusable lesson of the session.

**① HON COVERED — the blocker from 8/11 is CLOSED.** Confirmed from run `33548106772`: `armed=True breach=False short_breach=False last_seen_book=0 longs=0 shorts=0` → `OK — armed and flat (book=0)`. The live `wind_down_state.json` shows `open_shorts: []` with `short_acked: []` and `flat: true`, so this is a **real cover, not an acknowledgement** — had `QT_FLAT_ACK_SHORT` been used, `short_acked` would list HON and `flat` would still be false. Every trading run since 9/01 13:35Z is green (8 consecutive). The last red was 9/01 00:40Z.

⚠️ **Consequence worth recording:** the `Predictions ledger guard` was deliberately placed *beneath* the permanently-red flat gate so a standing red could not swallow it (PR #25). That arrangement is **no longer load-bearing** — the gate is green now, and a future red means something new.

**② THE SUNSET WAS STILL ONE DAY SHORT — and 8/21 is a permanent hole.** `2665b4c` moved 9/25 → 9/26 to land obs #30, but its arithmetic counted **2026-08-21 as an observation**. That day has **ZERO rows in `predictions.csv`** — the unpinned `river` 0.26 break (`01b2897`). Verified directly: every trading day 07-30 → 08-31 carries 304–307 rows except 08-21, which has none.

The write rule is empirical — a row lands on the run **6 MARKET SESSIONS** after its pred-day (8/24→8/14, 8/25→8/17, 8/27→8/19, 8/28→8/20, 9/01→8/24). The lag counts *sessions*, but the observation count only advances on days that *have* predictions. That is why the 8/31 run wrote **no row at all**: 8/31 minus 6 sessions lands exactly on the 8/21 hole.

✅ **CONFIRMED LIVE the next morning:** the 9/01 run wrote pred **08-24** and **skipped 08-21 outright** rather than backfilling it — so the hole is permanent and the correction is not theoretical. With 8/21 excluded and Labor Day 9/07 removed, obs #29 is pred 9/17 (written 9/25) and **obs #30 is pred 9/18, written by the 9/28 run**. Sunset moved **9/26 → 9/29** in all three workflows (PR #28, merged). **Last run day is Mon 2026-09-28.**

**③ S4 now reads N=12: +0.0128, t=0.37.** For t≥2.0 at N=30 with the observed daily spread (sd ≈ 0.119) the mean must clear **+0.0436** — the t-stat binds harder than the +0.03 floor. 12 days banked sum +0.154; 30 days need +1.307; so **the remaining 18 sessions must average +0.064/day**, five times the full-window mean and roughly the level of the single best 4-day stretch in the whole series, sustained for eighteen. The multiple-testing gate is also nowhere near: `SR=1.046 WRC_p=0.523 SPA_p=0.955 -> not significant`.

**④ v27 OPENED — and it OVERRULES `NEXT_ARCHITECTURE.md` §⑤ deliberately.** The user chose **full-auto execution on Alpaca** (paper → live). Robinhood stays a **manual** account and is explicitly NOT automated: it publishes no official equities API, and automating it needs private endpoints against its ToS. `docs/V27_BUILD_PLAN.md` records the decision and marks §⑤'s reasoning as *answered by the C-layer guards*, not withdrawn.

⚠️ **`NEXT_ARCHITECTURE.md` §⑤ named this exact request the live risk** ("rebound-building — starting again because stopping feels like losing"). It was raised once, the user reaffirmed, and work proceeded. Recorded here so the override is visible rather than implicit.

**⑤ THE PRE-REGISTRATION IS SIGNED AND BINDING** (`docs/V27_PREREGISTRATION.md`, PR #29 + #30). `P(success) = 15%` · `K = 5` · allocation `$10,000`. Four conjunctive criteria E1–E4 with dated reads (E1 **2026-12-15**, E2 1/15, E3 3/1, E4 concurrent), **one restart for the whole project**, the anti-deferral clause carried over verbatim, terminal **2027-03-31**, and a ramp in percentages (5→15→40→100 = $500/$1,500/$4,000/$10,000) so it cannot be re-based.

🔑 **E1 carries a stability clause v25 did not have:** the effect in the final third must be ≥50% of the first third. S3 produced a genuine +0.0254 over folds 1–6 that decayed to −0.0053 by folds 9–12 and still read as a near-miss. That failure mode is now a gate.

**⑥ EDGE LAYER BUILT — A2, A3, A4 all merged, all green.**

* **A2 `event_study.py`** (PR #31) — consumes events, not a cross-section. Look-ahead, fake independence and unsettled exits are made *unrepresentable*: entry is the first bar **strictly after** `event_ts`; overlapping same-ticker events inside the horizon are deduplicated (they share most of one price path and inflate t by ~√2); exits require a later bar to prove settlement. Output frozen on first write, keyed on `event_id`. **13 checks**, incl. a look-ahead probe where the correct answer is 0.0000 and a bug reads +0.5000.
* **A3 `extract_form4.py`** (PR #32) — **SEC EDGAR is a dead path from CI** (`quant_runner.py:6900`; confirmed again — SEC returns its "Undeclared Automated Tool" page). Finnhub instead, which costs the intraday filing time and makes next-session entry the only defensible read: **biases effects DOWNWARD by up to one session.** Code **P** only — the existing insider block infers buy/sell from the *sign* of the share-change field, lumping in awards, exercises and gifts.
* **A4 `build_universe.py`** (PR #33, #34, #35, #36) — Cell 2's `_ADV_MIN_USD = 50_000_000` floor becomes a **ceiling**. Point-in-time ADV uses only bars strictly before the as-of date (a screen reading post-event bars decides *membership* with the future — invisible to every guard in A2). Median not mean.

**⑦ THREE LIVE PROBES, THREE BUGS THAT PASSED THEIR OWN TESTS.** The reusable finding of this session:

1. **Zero open-market purchases in mega-caps.** 8 names, 365 days: 2,661 transactions, **26 code-P, 25 of them one ticker**, six of eight at zero. Mega-cap insiders are *compensated* in stock and sell it. A3 could not reach N≥40 on that universe at any parameter setting — which moved A4 from "later refinement" to **prerequisite**.
2. **Untyped symbols waved into the universe.** The funnel kept 19,088 "operating companies" against only 18,433 typed Common Stock. `if typ and typ not in COMMON_TYPES` let every blank-type row skip the check.
3. 🔴 **The screen returned ROCHE HOLDING AG as an illiquid small-cap.** `RHHVF`, ~$250B, qualified because its **US OTC line** is thin — along with `ATUUF`, `EXETF`, `CURLF`, `KRKNF`, `NOPMF`, `CDDRF`, 7 of 69 names. **ADV on an OTC foreign ordinary measures the thinness of its US listing, not the size of the company.** Independently, foreign private issuers are exempt from Section 16 (Rule 3a12-3(b)) and file no Form 4s at all — they can never produce an event. Fixed with a MIC allowlist, which then revealed **13,457 of 18,433 Common Stock rows (73%) were OTC**.

**⑧ THE UNIVERSE: 907 names.** Population 4,956 (every US primary-exchange common stock), 4,949 priced (99.9%), kept **907** — 697 XNAS, 160 XNYS, 50 XASE. ADV $250,036–$4,981,341, median $1,335,024. Funnel: `no_price 7, short_history 349, price_too_low 720, too_liquid 2,446, too_illiquid 527`. `too_liquid` being the largest bucket is the screen working: half the listed market trades above $5M/day, and that half is what v25 already measured.

**Sampling was SUPERSEDED, not merely enlarged.** The 1,500-name sample was chosen against a believed population of 18,413 — a figure 73% inflated by the OTC rows the MIC fix removed. Pricing all 4,956 takes ~4 minutes. 🔑 **Taking everything is methodologically better, not just cheaper: the seed and sample size stop being part of the specification.** Two free parameters ceased to exist. Free to do *now* because no performance data had been observed; it would have cost one of K=5 after an E1 read.

**⑨ ⚠️ TWO BIASES THAT DO NOT CANCEL** — both documented in `build_universe.py`:

* **SURVIVORSHIP, biasing UPWARD** — today's listed universe only. Delisted and acquired companies are absent, and insiders buy on the way down too. Small illiquid names are exactly where delisting is common, so the bias is **largest precisely where v27 is looking**. Finnhub's free tier exposes current listings only; **this is not fixed**, and any E1 pass must carry the caveat *with* the number.
* **NEXT-SESSION ENTRY, biasing DOWNWARD** by up to one session.

Different mechanisms, unknown relative magnitude. **Do not net them off.**

**⑩ IN FLIGHT AT SESSION END.** The event count over the 907-name universe (run `33579963301`) was still running — 907 tickers at Finnhub's 60/min is ~17 minutes. **Availability only: the job runs the extractor and stops, computing no returns.** Event *counts* are free against K=5; event *performance* is not, and they are kept in separate jobs so that boundary is structural rather than stated. The result was not seen this session.

⚠️ **When the count lands:** if it is short of 40, widening the ADV ceiling is **still defensible** — a count is not a performance read. After E1 returns a *return*, the same act consumes one of the five. That distinction is the whole reason the jobs are separate.

**⑪ HOUSEKEEPING.** PR #21 (`feat/maximize-model`, GPU/compute) **closed** — superseded by its own 6/14 validation (AUC 0.55 flat, "frame at ceiling, pivot to frames not compute"), and merging a model-behaviour change mid-S4 would be a dated change inside a live window. Branch preserved; the negative result is cited in `NEXT_ARCHITECTURE.md` §①.

⚠️ **SEQUENCING STILL BINDING:** nothing in v27 may touch the live pipeline before **2026-09-29**. Every v27 workflow is dispatch/branch-only, none is scheduled, none writes under `data/predictions/`, and each asserts the S4 evidence tree is untouched. `quant_daily.yml`'s most recent change remains the sunset fix.

## 🗓️ SESSION LEDGER — 2026-08-27 (Thursday): 📅 **S4's read date was one trading day off, and at the old sunset the series would have ended at N=29 against a pre-registered 30.** Sunset moved 9/25 → 9/26 in **three** workflows (the third was undocumented). ✅ All every-session health checks PASS; the ledger guard and the rank-IC freeze are both still holding. 🔴 **HON still uncovered — and `mode=execute` has NEVER been dispatched.**

**① 📅 THE S4 END DATE — CORRECTED, AND THE SUNSET MOVED TO MATCH.**

The estimate in this file was **~2026-09-24**, and the `sunset` gate was built around it (`QT_SUNSET_DATE: '2026-09-25'   # day AFTER S4's expected read (~2026-09-24)`). Both were one trading day short.

**The lag is measured, not assumed.** Mapping each run to the pred date it appended to `rank_ic_v2.csv`:

```
run 2026-08-24 (c7bb579) -> added pred 2026-08-14
run 2026-08-25 (ed510b8) -> added pred 2026-08-17
run 2026-08-27 (3ad6c10) -> added pred 2026-08-19
```

A steady **6-trading-session** lag (`HORIZON_DEFAULT=5` + `SETTLED_ONLY=1`). ⚠️ The 8/26 run added **three** rows (8/12, 8/13, 8/18) — that is the `8ac6d42` wipe recovery, a one-off; do not read it as the cadence.

**The miss was Labor Day.** With 9/07 closed, no prediction is logged, so the pred-date sequence loses a day. Counting from obs #10 (`2026-08-19`, today's newest), obs **#30 is pred 2026-09-17**, written **6 sessions later on the 9/25 run**. Obs #29 (pred 9/16) is the one that lands on 9/24.

**So at the old sunset the final read would have been N=29 — one short of the pre-registered 30.** Drop the holiday and #30 lands exactly on 9/24, which is almost certainly how the original estimate was built.

🔑 **This does NOT change the verdict and must not be read as reopening anything.** The series is +0.0133 with **t = 0.32** against a bar of **t ≥ 2.0**; one more observation of a ~0.13-std daily IC cannot move that. The change exists solely so the criterion is met **exactly as pre-registered** — "N=29 against a rule that says 30" is precisely the technicality §"STOPPING RULE" exists to stop someone arguing from later.

**② 🔑 THE SUNSET DATE LIVES IN THREE FILES, NOT TWO — the "both files" note in the checkpoint table was wrong.**

`grep -rn QT_SUNSET_DATE .github/workflows/` returns **three** gate values:

| File | Line | Effect if left unmatched |
|---|---|---|
| `quant_daily.yml` | 73 | `trade` job skipped — the actual retirement |
| `morning_watchdog.yml` | 67 | pages Discord **every weekday forever** over an expected absence |
| `universe_watch.yml` | 62 | posts to Discord **every weekday forever**, same failure mode |

`universe_watch.yml` shipped 8/26 (`706d871`/`d4574f7`, PR #27) with its own copy of the gate and a correct "MUST match" comment, but the checkpoint row in this file still said "**change `QT_SUNSET_DATE` in both files**". Anyone following that instruction would have left the universe watch crying into Discord from 9/26 onward. **Row 214 now names all three.**

**③ ✅ EVERY-SESSION HEALTH CHECK — ALL PASS on the 8/27 morning run `33077586901`.**

`MORNING cycle complete -- 2026-08-27 15:54 UTC` (inside 13-20Z), `Run type: morning`, `ET='2026-08-27'`, `Training complete: 308 models trained, 0 skipped` (the `01b2897` river pin holding a fourth morning: 306/307/307/**308**), `[patch] Gross cap: … cap 0.00x -> 0 new BUY slots | pre-blocked 19/19 BUY signals`, `[patch] Oversell guard: enforce=True pos_map=1 pos_ok=True`, `[patch] Sector cap: … ok=True | pre-blocked 0/19`, kill switch quiet at `daily_dd=-0.12%`.

✅ **The ledger guard reported green from under the red gate again** — `OK — ledger did not shrink (51313 -> 51618)`, **+305**. ✅ **The rank-IC freeze is holding** — `FROZE 22` / `10` / `100` / `22`, originals kept in every case. 🔴 The only red is the expected one: `short_breach=True book=1 longs=0 shorts=1`, HON. **`breach=False` / `longs=0`** — the long-side re-entry detector still reads clean and independently.

**④ 🔴 HON — `mode=execute` HAS NEVER BEEN DISPATCHED. Not once.**

Checking `TRIM_MODE` on **every** `position_trim.yml` run in the workflow's history:

```
33040565293 dry-run   33032587522 dry-run   33004162776 dry-run   32927896291 dry-run
32418542070 dry-run   32369620774 dry-run   31646381986 dry-run   30670917205 dry-run
```

⚠️ **Eight for eight.** The cover has been "one command away" since 8/12 and that command has never been run. Latest state (`33040565293`, 8/27 04:49Z): `HON short=-13 openBUY=0 price=219.94 cover=13 cost=$2,859 P&L -1,226` — cost drifted up from $2,808 as HON rose 216.00 → 219.94. The printed P&L stays a distorted-basis artifact; do not book it.

🔑 **This is a trade, so Claude cannot fire it — that is a hard constraint, not a preference, and re-asking will not change it.** Every session hits the same wall and returns another dry-run; three were run on 8/26-8/27 alone. **The only thing that clears it is the user dispatching `-f mode=execute -f sleeve=short-cover` personally.**

**⑤ 🔑 THE REPO HAS MORE THAN ONE LIVE CLAUDE SESSION — a commit appeared mid-audit and it was not automation.**

`HANDOFF.md` was committed (`42e5d51`) and pushed (`d8772fd`) at 19:09-19:10 ET while this session was reading it, which initially looked like an unattended writer of the class that caused the 7/06 Drive-sync and 7/07 Task-Scheduler incidents. **It was not.** Ruled out by direct check: **no git hooks** (only `.sample`), **no `hooks` key** in project or user `.claude` settings, **`trigger_cycle.ps1` contains zero git commands** (`QT-Evening` ran 19:11:07, *after* the push), no other scheduled task near the time, and CI `pull --rebase` runs on GitHub runners and cannot write this machine's reflog. `pull.rebase false`, no aliases.

The actual source: **Claude Code session `c1688512-5e4e-438e-974e-3aaf5a0588a1`**, open since 8/26 18:44Z and still alive at 23:11Z, whose transcript holds the `git commit -q -F -` heredocs for all three recent docs commits (ledger ⑪-⑫, `NEXT_ARCHITECTURE.md`, ledger ⑬). Its last user turn — *"save this to the hand off"* at **23:08:13Z** — precedes the commit by 87 seconds.

⚠️ **Operational note: when two sessions are open on this repo, either can commit and push under the other's feet.** Check `git status` before and after any commit, and prefer read-only work if another session may be mid-task.

---

## 🗓️ SESSION LEDGER — 2026-08-26 (Wednesday): ✅ **THE STALE-BASE FIX IS PROVEN — the ledger guard reported green from *underneath* the permanently-red flat gate, and the exact 8/24 race recurred and did not bite.** ✅ **The recovery paid out: S4 went N=6 → 9 (better than the forecast 8) and the two recovered days came in strongly POSITIVE, flipping the mean's sign.** 🛑 **STOP CRITERION S2 HAS LANDED AND IT FAILS — Frame 2 is at 32 IC obs with t = −1.17 against a `t < 1.0 → RETIRE` bar. Three of the four terminal criteria are now down.** ⚠️ **The runner prints `Frame 3 gate: PASS` while S1 as pre-registered FAILS — the script's PASS covers drawdown and beta only.** 🔴 **HON still uncovered — now the ENTIRE book; cover ~$2,808, unrealized −$1,174.**

**① ✅ THE STALE-BASE FIX IS PROVEN — CLOSE THE WATCH.** All three predictions from the 8/25 ledger ⑥ held, and the fourth beat its forecast.

*(a) The guard ran, and it ran where it could be seen.* Morning (`32975067090`, 13:35Z→16:02Z, state commit `9464778`):

```
Flat-invariant gate (fails the run if the book is not empty)   ✗
Predictions ledger guard (row count must never decrease)       ✓
```

`ledger guard: origin/master tip=51313 rows, parent=51008 rows` → `OK — ledger did not shrink (51008 -> 51313)`. **+305.** Every later run of the day repeated it flat at `51313 → 51313`. 🔑 **That ✓ printed directly beneath a ✗ is the entire design point** — the guard was placed after the flat gate precisely so HON's permanent red could not swallow it, and this is the first live confirmation that the placement works.

*(b) The adversarial case recurred and was defeated.* Run `32982441632` was **created 14:47Z** — inside the morning cycle — and, queued behind it, did not start until **16:05Z**. That is *exactly* the shape of `32744956711` on 8/24. Under the pinned-`github.sha` checkout it would have read a pre-morning tree and rewritten `predictions.csv` off it. It instead resolved the live tip:

```
Morning marker: origin/master='2026-08-26' checkout='2026-08-26' -> using '2026-08-26'
Run type: intraday (UTC hour=16 day=3)
ledger guard: origin/master tip=51313 rows, parent=51313 rows
```

Same for `32986866050` (created 16:07Z). 🔑 **This is a real test, not a replay** — the race fired on its own schedule and the fix held. Two independent runs cleared it.

*(c) The morning's own rows survived* every intraday commit through 18:18Z. Local `master` at `c9baa42`: `wc -l data/predictions/predictions.csv` = **51313**.

**② ✅ THE RECOVERY PAID OUT — AND IT MOVED THE SIGN.** `rank_ic_v2` went **N=6 → 9**, one better than the forecast 6 → 8 (8/18 settled the same session rather than waiting for 8/27). The 8/26 morning appended three rows, two of them the days recovered by `8ac6d42`:

| date | n | rank_ic | |
|---|---|---|---|
| 2026-08-12 | 278 | **+0.1623** | ← recovered |
| 2026-08-13 | 278 | **+0.0804** | ← recovered |
| 2026-08-18 | 276 | +0.0556 | |

🔑 **Both recovered days came in strongly positive and carried the S4 mean across zero** — −0.0268 (N=6) → **+0.0153 (N=9)**. Read that as *the 8/25 read was contaminated by the wipe*, **not** as a signal that anything is reopening: t = 0.33 against a bar of 2.0 is not close, and the anti-deferral rule means a favourable wobble is not a reason to re-litigate. 8/24 still matures around 9/02.

**③ 🛑 STOP CRITERION S2 HAS LANDED — AND IT FAILS. FRAME 2 RETIRES.** S2 was due ~Fri 8/21 and pre-registered as **FINAL when it lands** (the instrument was audited clean on 8/06, so there is no instrumentation escape hatch). The 8/26 scorecard crosses the 30-observation threshold:

```
=== Frame-2 gate scorecard (intraday shadow, open->close) ===
days (N)      : 32 IC obs / 32 L/S rows
mean rank-IC  : -0.0139   t-stat -1.17   [gate: >= 0.03 & t >= 2.0  -> NOT YET]
trailing-20d  : -0.0267   t-stat -1.85   (n=20)
mean L/S ret  : -0.0009   cumulative -3.01%
Sharpe (ann.) : -2.06     %win days : 12/32 = 38%
beta vs SPY   : -0.29   [gate: |beta| < 0.2  -> FAIL]
Frame 2 gate  : FAIL (rank-IC, beta) — full read at >= 30 IC obs (~0 trading days away)
```

**The S2 bar is `rank-IC t < 1.0 → RETIRE the frame`. t = −1.17.** It fails on sign as well as magnitude, the trailing-20d read is *more* negative than the full window (−0.0267, t −1.85 — decaying, not converging), and the beta gate fails independently. **Per the pre-registration this is decided: Frame 2 is retired.** Do not extend the window and do not rebuild the intraday feature set — S2 says "RETIRE", not "iterate". Update the criteria table row and treat Frame 2 as measurement-only history from here.

**④ ⚠️ THE RUNNER PRINTS `Frame 3 gate: PASS` AND S1 STILL FAILS — DO NOT CONFUSE THEM.** The stat-arb scorecard now has N=40:

```
=== stat-arb gate scorecard (Frame 3 shadow book) ===
days (N)      : 40  (2026-06-30 -> 2026-08-26)
mean daily    : -0.0627%   cumulative -2.52%   net P&L $-2,007 (costs $250)
max drawdown  : -3.4%      [gate: > -15%  -> OK]
Sharpe (ann.) : -2.08   [report-only — no blind threshold committed]
beta vs SPY   : +0.04   [gate: |beta| < 0.2  -> OK]
Frame 3 gate  : PASS on committed gates
```

🔑 **That `PASS` covers drawdown and beta ONLY.** The script says of Sharpe *"report-only — no blind threshold committed"* — but **S1 in this document DOES commit one: annualised SR < +0.5 → RETIRE.** At **SR −2.08 on 40 observations**, S1 fails by a wide margin, exactly as it did at the 8/12 read (−0.84 on 27). Closed trades tell the same story: 11 closed, 45% win, **8 of them exited `decointegrated` for −$1,944** against 3 reversions for +$1,152 — the pair selection is decaying faster than the spreads revert. **A future session grepping for `Frame 3 gate` will find the word PASS; it does not clear S1.** This mismatch between the script's vocabulary and the pre-registration's is a documentation hazard, not a code bug — the script was written before S1 committed a threshold.

**⑤ ✅ THE S4 "MOVING TARGET" HAZARD IS CLOSED — RETRACT THE 8/10 WARNING.** The 8/10 ledger recorded that `analyze_rank_ic.py` full-overwrites its outputs and restates history each morning, and instructed that the log line *"Rows already written never move"* be treated as **false**. **That is no longer true.** `_freeze_first_write()` (`analyze_rank_ic.py:132`) landed in `203d95c` on 8/21 and is applied to **both** outputs — `rank_ic` at `:374` and `cross_sectional_ls` at `:481`. It diffs each recompute against what is already on disk and **keeps the original**, announcing every intervention. The 8/26 run:

```
[rank-ic] rank_ic: FROZE 22 recomputed value(s) that would have changed
          already-written rows — kept the originals: 2026-07-14.n 279->278; ...
[rank-ic] rank_ic (v2): FROZE 10 ...  2026-08-06.rank_ic -0.1219->-0.1175; ...
[rank-ic] cross_sectional_ls: FROZE 19 ...
```

🔑 **Written rows really are immutable now, and the freeze is loud about what it caught.** The ~9/24 S4 read no longer needs snapshotting — still report it **with its read date**, but it is an accumulated ledger, not a recompute. ⚠️ Note the second-order caveat already in the source at `:477`: the newest row is computed against frozen predecessors, so it is exact for the gate inputs (`long_ret`, `short_ret`, `long_short`) and only approximate elsewhere.

**⑥ 📊 WHERE S4 STANDS — read date 2026-08-26.**

```
--- summary (FULL window) ---
days (N)      : 9        window : 2026-08-06 -> 2026-08-18
mean rank-IC  : 0.0153   [gate: >= 0.03]     t-stat : 0.33   [gate: >= 2.0]
% days IC > 0 : 56%      std daily IC : 0.1386
Stage-1 IC gate (full) : NOT YET
```

Ranking-variable health is fine — 3.4% tied at the modal value, ~269 of 278 names carrying a distinct value, 0/14 days with no name below 0.5 — so this is a *valid* cross-section, unlike the v1 `confidence` series (8/25 ledger ⑧). **Both beta gates still FAIL:** raw L/S beta vs SPY **−1.27** (corr −0.76), residual beta on hedged rows **+2.24** — but that is only **3/9 rows hedged**, still inside the warm-up where beta is unidentified. **Quote the raw leg, not the hedged one.**

**⑦ 🔎 THE CROSS-SECTION IS 305 TODAY, NOT 307 — flag, not alarm.** The morning logged `0 trades executed | 305 predictions logged`, and the ledger grew by exactly **+305**, so the file is internally consistent. `[Tier3] DSR filter: 305/305` agrees. This is **not** the same number as `Training complete: 307 models trained, 1 skipped`, which counts per-ticker intraday models and is a different population — do not read the two against each other. Two names dropped out of the equity cross-section versus 8/25. Worth a glance next session for a delisting or a data gap; nothing about it moves a gate.

**⑧ 🔴 HON — UNCHANGED, AND IT IS NOW THE ENTIRE BOOK.** The 03:49Z dry-run (`32927896291`, `mode=dry-run`, nothing submitted):

```
Account: equity=$113,781.55  cash=$116,589.55
Positions: 1 total, gross $2,808 (0.02x) | 1 SHORT ($2,808)
  HON  -13  openBUY=0  price=216.00  cover=13  cost $2,808  P&L -1,174
── Plan: 1 market BUY-to-cover (DAY, fill at next open), ~$2,808 ──
Projected after covers: gross ~$0 (0.00x equity), zero shorts
DRY-RUN — nothing submitted. Re-dispatch with mode=execute.
```

All three safety conditions still hold: **not an empty read**, qty **exactly 13**, `openBUY=0` so **no pending buy to double-cover**. The gate names it directly — `SHORT BOOK — 1 short position(s) the wind-down cannot close: HON. Cover with position_trim.yml sleeve=short-cover, or acknowledge via QT_FLAT_ACK_SHORT`. ⚠️ The printed `P&L −1,174` is still the **distorted-basis artifact — do not book it** (8/12 ledger caveat). **This is a trade: the user fires it, not Claude.** Run tally since 8/12 is now **77 failures / 33 cancelled, zero successes** on `quant_daily.yml`.

**⑨ 📌 SCOREBOARD — THREE OF FOUR TERMINAL CRITERIA ARE DOWN.**

| | Frame | Status after 8/26 |
|---|---|---|
| **S1** | Frame 3 — stat-arb | 🛑 **FAILED** — SR **−2.08** on N=40 vs a **+0.5** bar (was −0.84 on 27). The script's `PASS` is drawdown/beta only — see ④. |
| **S2** | Frame 2 — intraday | 🛑 **FAILED TODAY** — t **−1.17** on N=32 vs a **t ≥ 1.0** bar. Read is FINAL. **RETIRE.** |
| **S3** | The label experiment | 🛑 **FAILED 8/09** — +0.0154, t 2.75, missed the +0.02 magnitude; effect decays to zero in recent folds. |
| **S4** | Frame 1 v2 | ⏳ **PENDING ~9/24** — N=9, **+0.0153**, t **0.33**. Sign is now right; magnitude and significance are not. |

🔑 **Nothing here reopens anything.** S4 improving is the *expected* consequence of un-erasing three prediction days, and one favourable read on N=9 is not a Stage-1 pass. The bar remains **+0.03 AND t ≥ 2.0, and it must also clear WRC/SPA**. Terminal date **2026-09-30** stands.

**⑩ 🔴 THE CROSS-SECTION SILENTLY VARIES IN SIZE, AND IT REACHES INTO THE S4 WINDOW.** ⚠️ **The count in this item is a day-over-day DELTA and understates the absolute picture — it is 9 tickers, not 2. Read ⑪ with it.** Chasing ⑦'s "305 not 307" turned up a **fifth silent-failure of the familiar shape**. The universe is not fixed at 307 — it has been drifting all month, and nothing records it:

| pred-day | size | missing |
|---|---|---|
| 2026-07-14 → 08-14 | 307 | — (22 consecutive days) |
| **2026-08-17** | 306 | `DOGE-USD` |
| **2026-08-18** | 306 | `DOGE-USD` |
| **2026-08-19** | **304** | **`AVB`, `EQR`**, `DOGE-USD` |
| **2026-08-20** | **304** | **`AVB`, `EQR`**, `DOGE-USD` |
| 2026-08-24 / 08-25 | 307 | — |
| **2026-08-26** | **305** | **`AVB`, `EQR`** |

🔑 **This is not random flakiness — it is the SAME PAIR recurring.** `AVB` and `EQR` (both residential REITs) dropped together on **three** separate days, and neither is delisted; both carry normal prices on every other session. `DOGE-USD` is crypto and is excluded from rank-IC anyway, so it is harmless — **but `AVB`/`EQR` are equities and DO sit in the measured cross-section.**

**The mechanism, traced end to end.** The Cell-2 download does a batch pull, then a parallel fallback, then a keep/skip loop. Today: `Batch downloading 317 tickers` → `308 downloaded via batch` → `Parallel fallback for 9 missed tickers` → **no `parallel fetch failed` line at all**, then `SKIP EQR` / `SKIP AVB`. There are **two silent fall-through points**:

```python
_df = _fut.result()
if _df is not None:            # <-- _df None: nothing logged, nothing stored
    all_prices[_tk] = _df
...
if df is not None and len(df) > 100:
    ...  print(f"  OK {tk:<10} {len(df)} rows  ${...}")
else:
    print(f"  SKIP {tk}")      # <-- no reason, no row count, no exception
```

Compare the sibling branch one line up, which *does* report itself: `DELISTED? {tk} {len(_recent)} rows in last 30d -- skip`. **The bare `SKIP` is the only outcome in that loop that explains nothing** — you cannot tell from the log whether the fetch returned `None`, returned 40 rows, or raised and was swallowed.

**Why it matters, stated honestly:** the effect on any number is **small** — 2 names out of ~278, well under 1%, and it does not plausibly move a t-stat of 0.33. **This is not a result-changing bug and it is NOT grounds to reopen anything.** What it is, is a **measurement-provenance hole**: `n` per day is an input to every rank-IC read, this document already instructs that `n` shifts be attributed to dated model changes (e.g. `65be103`, `WYFI`, 279 → 280), and there is now a known **undated, silent, intermittent** source of the same drift that no artifact records. After the fact you cannot reconstruct which names were in the universe on a given day except by diffing `predictions.csv` — which is what this item had to do.

✅ **ACTIONED SAME SESSION — `84149f7` (PR #26) + `e91b132` (PR #27).** The logging fix and the watcher both shipped; see ⑪ and ⑫.

**⑪ 🔴 CORRECTION TO ⑩ — IT IS 9 TICKERS, NOT 2, AND 7 OF THEM ARE CHRONIC.** ⑩ reported *"2 names dropped"*. That is correct as a **day-over-day delta** and badly understates the absolute picture. Dry-running the new watcher's extraction against the 8/26 log returned **9 bare `SKIP` lines**:

```
ANSS  AVB  EA  EQR  HOLX  IPG  K  MMC  SQ
```

| ticker | in predictions 8/25 | 8/26 | |
|---|---|---|---|
| `AVB`, `EQR` | ✅ | ❌ | the intermittent pair from ⑩ |
| `ANSS`, `EA`, `HOLX`, `IPG`, `K`, `MMC`, `SQ` | ❌ | ❌ | **chronically absent — duration unknown** |

🔑 **Seven tickers have been silently missing for an unknown length of time**, and the only reason ⑩ did not see them is that it diffed 8/26 against 8/25 — where they were *already* gone. **A delta cannot find a constant.** They match yfinance errors in the same log (`$ANSS: possibly delisted; no timezone found`, `$EA: possibly delisted; no price data found`, likewise `$HOLX $IPG $K $MMC $SQ`), and several are plausibly genuine corporate actions. **The nominal 317-name watchlist is effectively ~305 and nothing says so.**

**And there is a real logic gap underneath.** The `DELISTED?` branch — the one that actually prunes `WATCHLIST` — sits **inside** the has-data arm:
...
if df is not None and len(df) > 100:
    ...
    if len(_recent) < 5:
        print(f"  DELISTED? ...")
        _delisted_tickers.append(tk)     # <-- only reachable WITH data
        continue
    ...
else:
    print(f"  SKIP {tk} ...")            # <-- no data at all lands here
```

So `_delisted_tickers` can only catch a ticker that **still returns >100 rows** but has gone stale in the last 30 days. **A ticker whose fetch returns nothing at all bypasses delisting handling entirely** — skipped forever, silently, and `WATCHLIST` is never pruned. That is why these seven never appeared as `DELISTED?`.

⏭️ **NOT FIXED, and deliberately so.** Pruning or re-mapping those tickers (`SQ`-style renames, M&A successors) **changes the equity cross-section**, which makes it a **dated MODEL change** under this project's own rules — not something to slip into an observability PR. **Decide it explicitly, or leave it: with the search stopped, the honest option is to leave the universe exactly as it has been all along rather than alter the cross-section during a measurement window.** ⚠️ If it is ever changed, it becomes the 10th dated model change and every `n` comparison across that date breaks.

**⑫ ✅ BOTH HALVES SHIPPED — `84149f7` (PR #26) and `e91b132` (PR #27).**

*PR #26 — make the failure legible.* Three logging-only changes to Cell 2: the parallel fallback's `if _df is not None:` had **no else** (a `None` result was stored nowhere and logged nowhere); `_skipped_tickers` now accumulates alongside `_delisted_tickers`; and the bare `SKIP` now reports **which condition fired and the row count**, matching its `DELISTED?` sibling, plus a closing WARNING naming every skipped ticker. **NOT a dated model change** — membership, features, signals and sizing untouched.

🔑 **The preflight half is the more important half.** Checks 1 and 2 `py_compile`/`ast.parse` every `.py` file — **but nothing parsed `trading_model_v25.1.ipynb`, the file that actually trades.** A syntax error in a notebook cell passed every preflight check and surfaced only at runtime, in a live morning run, as a broken retrain — **precisely the 8/21 river-break shape**, which cost a Frame-1 evidence day inside the S4 window. New **check 12** AST-parses all 23 code cells (blanking 3 magic lines); new **check 13** `json.load`s the notebook and pins cell structure at **24 total / 23 code**, because `quant_runner` indexes cells **positionally** (`SKIP_BY_TYPE`, `CELL_TAGS`) and a cell appearing or disappearing silently re-points every skip list. Preflight `33017822316` **all 13 PASS**.

*PR #27 — read it every session.* `.github/workflows/universe_watch.yml`, 17:45Z weekdays, posts the composition report to Discord. Carries the **sunset gate matched to `quant_daily.yml`** (without it it would post every weekday forever after 9/25 about an expected absence), identifies the morning run by the resolver's own `Run type: morning` line rather than by timestamp or duration, includes a **bare-`SKIP` regression detector**, and **always exits 0** — it reports, it never gates.

⚠️ **Two process traps worth carrying forward, both hit while doing this:**
* **`workflow_dispatch` on a branch is refused** — `404: not found on the default branch`. A new alerting workflow **cannot be smoke-tested before merge**; only its YAML parse is covered (preflight check 10). Budget for a first-run fix.
* **Editing the minified `.ipynb` with `perl -pe s///` corrupted it twice** — the replacement turned `\n` *escapes* into real newlines, producing a file that still looks right in a diff but is no longer valid JSON. Restored from backup (md5 verified) and redone with exact-byte `index`/`substr` splicing, each edit asserted to match **exactly once**. **Do not regex-edit this file; splice it.** Check 13 now catches the failure mode.

**⑬ 📊 THE CONSOLIDATED VERDICT, AND WHAT IS QUEUED — session tail, 2026-08-27 ~02:40Z.**

**(a) 🛑 LEADING VERDICT: NO-GO. RETIRE THE ARCHITECTURE.** Not close, and not a marginal call. The **six-gate real-money deployment gate** (§ REAL-MONEY DEPLOYMENT GATE) requires all six to hold **simultaneously**; four fail outright:

| Gate | Threshold | 2026-08-26 read | |
|---|---|---|---|
| Shadow rank-IC | ≥ 0.03 **and** t ≥ 2.0 | **+0.0153, t = 0.33** | ❌ |
| Measurement window | ≥ 6 weeks | **N=9 (~1.7 wks)** | ❌ matures ~9/24 |
| Walk-forward OOS AUC | holds ≥ 0.55 | **0.4862** | ❌ **below coin flip** |
| Shadow L/S max drawdown | < 15% | −0.8% | ✅ on 9 days only |
| Beta to SPY | \|β\| < 0.2 | **−1.27** | ❌ |
| White Reality Check p | < 0.10 | **0.497** | ❌ |

**(b) 🔑 THE MODEL'S OWN THREE SELF-CHECKS ALL AGREE — quote these verbatim, they are the least arguable numbers in the project:**

```
[walkforward] 12 folds | mean OOS AUC=0.4862 | mean IC=-0.0376
              | last AUC=0.4528 | weak/no edge
[Tier3] DSR filter: 305/305 models flagged as likely false discoveries (DSR<0)
[Tier3] White Reality Check: SR=0.872 WRC_p=0.497 SPA_p=0.960 -> not significant
```

Walk-forward is **below random** with a **negative** mean IC and only **4 of 12 folds** clearing 0.50; the deflated-Sharpe filter rejects **305 of 305** of its own per-ticker models; WRC is a coin flip. ⚠️ **The same log also prints `305 tickers | 226 above 0.55` — that is the PER-TICKER figure and it is the known-flattering one.** The panel number (`0.4862`) is the baseline adopted 7/14 after root-causing exactly that inflation. **Quote the panel.**

⚠️ **PRECISION ON "FIVE INDEPENDENT NULLS" — that phrasing is too strong and should not be repeated.** S1 (pairs) and S2 (intraday) are genuinely separate frames, but **walk-forward, DSR and WRC are NOT independent of S4** — they measure the *same* Frame-1 model from different angles. The correct statement: *two independent frames found nothing, and the main frame's own model reports no predictive power under three different instruments.* That is still a NO-GO; it is a more defensible one.

**(c) 🔑 WHAT S4 WOULD HAVE TO DO.** From N=9 / t=0.33, reach **+0.03 with t ≥ 2.0** in ~4 weeks **and** clear WRC (currently p=0.497). That requires the cross-sectional ranking to carry strong signal *while the same model's walk-forward sits below random*. Not impossible — rank-IC and AUC measure different things — but it would be a **surprising result demanding explanation, not a quiet pass.** **Let S4 land anyway:** it costs nothing, the crons run themselves to the 9/25 sunset, and closing it properly is what makes the pre-registration mean anything.

**(d) 🔴 HON — THREE DRY-RUNS TONIGHT, ALL IDENTICAL, ALL CLEAN.** `33004162776` (19:15Z), `33032587522` (02:13Z), `33040565293` (02:38Z):

```
Account: equity=$113,730.38  cash=$116,589.55
Positions: 1 total, gross $2,859 (0.03x) | 1 SHORT ($2,859)
  HON  -13  openBUY=0  price=219.94  cover=13  cost $2,859  P&L -1,226
```

Safety conditions hold in all three — **names HON, qty exactly 13, `openBUY=0`.** The two overnight runs are byte-identical because the market is shut; `219.94` is the last trade and **cannot move until the 13:30Z open**, so further dry-runs before then add nothing. ⚠️ `P&L −1,226` remains the **distorted-basis artifact — do NOT book it** (8/12 ledger). 🔑 **The remaining uncertainty is the OPEN PRICE, which no dry-run can show.** ⏰ **Fire before 13:30Z** — inside 13:35Z–~16:05Z the trim queues behind the morning cycle on the shared `quant-terminal` concurrency group (`cancel-in-progress: false`) and the fill could slip a day. **Still a trade the user fires.**

**(e) ⏳ QUEUED, RUNS WITHOUT A SESSION.** A one-shot scheduled task **`qt-universe-watch-first-run`** fires **2026-08-27 14:15 ET** to verify the first live run of `universe_watch.yml` (17:45Z) and the first live exercise of the edited Cell 2. It asserts: **`bare SKIPs : 0`**, SKIP lines carrying reasons, `discord HTTP 204`, ledger guard OK, and the cross-section size. ⚠️ **It checks that fields are POPULATED, not merely absent** — the workflow could not be smoke-tested pre-merge (GitHub 404s `workflow_dispatch` off the default branch), so a broken grep fails as an *empty field* that reads exactly like a clean day. ⚠️ Scheduled tasks run only while the Claude app is open; otherwise it catches up on next launch.

**(f) 📄 FORWARD SKETCH WRITTEN — `docs/NEXT_ARCHITECTURE.md` (`8effe14`).** Design only: nothing scheduled, no code, gated on S4 landing and the closing ledger, and it opens by stating **it is not permission to restart.** Core finding: **the inputs were never the problem.** The 8/09 decision named "change the inputs" the only high-ceiling option — and `quant_runner.py` already has SEC EDGAR full-text + submissions, Quiver **short interest** and **options**, congress/insider trading, Finnhub, FRED, Ken French and PatentsView wired, **every one flattened into a scalar feature.** A Form 4 reading *"CFO bought $2M, first purchase in six years"* becomes `0.31`. Proposes an **instrument panel, not a predictor** (filing watcher, filing diff, constraint screen, base-rate lookup, thesis ledger), on the grounds that every autonomous predictor fights the efficiency prior head-on — the fight just lost. **Read that file before proposing any successor project.**

---

## 🗓️ SESSION LEDGER — 2026-08-25 (Tuesday): 🔴 **A SECOND SILENT EVIDENCE-EATER, OLDER THAN THE RIVER BREAK — THREE MORNING PREDICTION DAYS WERE WRITTEN AND THEN ERASED ~18 MINUTES LATER.** `actions/checkout` pins to `github.sha` at run *creation*, so intraday crons created during the ~2.5h morning cycle rewrote `predictions.csv` off a pre-morning tree and `rebase -X theirs` let the stale copy win. 8/12, 8/13 and 8/24 lost 307 rows each. ✅ **Root-caused to the second, rows RECOVERED from git (`8ac6d42`, +921 rows, 0 deletions), and both the cause and a detector shipped the same session** (`86947a6`, PR #25). ✅ **The 8/22 river fix is PROVEN** (8/24 trained 306, 8/25 trained 307). ✅ **The suspected 8/18–8/19 rank-IC gap is NOT a bug** — it is the designed settle lag. 🔴 **HON still uncovered — cover now ~$2,800, unrealized −$1,166.82 (−71.4%).**

**① ✅ THE 8/22 RIVER FIX IS PROVEN — CLOSE IT.** Both post-fix mornings retrained for real. Cell 2 ran, `build_features` produced `70 features` on `1971 rows`, and predictions were logged:

| | 8/24 (`32733549532`) | 8/25 (`32854214277`) |
|---|---|---|
| Cell 2 / `build_features` | OK, 70 features, 1971 rows | OK |
| Predictions logged | `307 predictions logged` | `307 predictions logged` |
| Intraday models | `306 models trained, 2 skipped` | `307 models trained, 1 skipped` |
| State commit | `c7bb579` pushed | `ed510b8` pushed |

`dsr_scores.json` also self-healed — **5546 bytes** at `data/predictions/dsr_scores.json` (note the path: it is *not* directly under `data/`, which is easy to mis-grep and read as still-gutted). ⚠️ The `STATE COMMIT LOST` strings in these logs are the workflow's own **script echo**, not output — the real line is `State commit pushed:`. Do not re-open that alarm on a grep hit.

**② 🔴 THREE MORNING DAYS WERE WRITTEN, COMMITTED, AND THEN ERASED.** The morning commit each day carried a full 307-row set. The **first intraday commit of the day**, 15–20 minutes later, reverted `predictions.csv` to a snapshot that predated it:

| Date | Morning commit | Erased by | Rows |
|---|---|---|---|
| 8/12 | `2ac8fc2` 16:06 → **307** | `6b76649` 16:24 | **0** |
| 8/13 | `c6719dd` 16:00 → **307** | `b289713` 16:16 | **0** |
| 8/24 | `c7bb579` 16:03 → **307** | `46be028` 16:21 | **0** |

On 8/24 the file went **50087 → 49780 lines** and its last row fell back to `2026-08-20 15:46:31`. The wipe also took `ticker_accuracy.json`, `learned_rules.json` and `ticker_calibration.json` with it. This is **not** the river break and **predates it** — 8/12 and 8/13 are two weeks earlier.

**③ 🔑 ROOT CAUSE — THREE CONDITIONS, ALL REQUIRED.** None of them is the resolver alone:

1. **`actions/checkout` pins to `github.sha`, which is frozen when the run is CREATED, not when the job starts.** Run `32744956711` was created **15:27:22** and did not check out until **16:03:55** — *32 seconds AFTER* morning commit `c7bb579` landed at 16:03:23 — and still got **`34d2c4f`, the 8/22 tip**. The morning cycle runs ~2.5h, so every intraday cron created inside that window starts from a pre-morning tree.
2. **The scorer does a read-modify-write of the *entire* `predictions.csv`** (`quant_runner.py:5323`, `_df14.to_csv(_preds14)`). On 8/24 it reported `Saved 604 scored outcomes` — it read the stale 8/20-ending file, scored it, and wrote the whole thing back, producing a genuine diff.
3. **`git rebase -X theirs origin/master` favours the *replayed* commit** — the runner's stale local file — over upstream. The workflow comment calls this "ours-wins"; from master's point of view it is **stale-wins**.

**④ 🔑 WHY 8/25 SURVIVED — LUCK, NOT IMMUNITY.** Conditions 1 and 3 were **both present and identical** on 8/25 (`headSha=599f5bc`, same `rejected → rebase → push`). Only condition 2 was absent: the scorer printed `No mature unscored predictions (need 5+ days old)` and never touched the file, so it never entered the conflict. Confirmed by diffstat — 8/25's intraday commit touches 5 files and `predictions.csv` is **not** among them; 8/24's touches 14 and it **is**. 🔑 Do not read 8/25's survival as evidence the race is intermittent by nature; it fires whenever the scorer has work to do.

**⑤ ✅ ROWS RECOVERED — `8ac6d42`.** The erased rows were never lost from *history*, only from the tip. Lifted each day's 307 rows out of the morning commit that still held them and inserted them at their chronological position. **Purely additive: 921 insertions, 0 deletions.** Verified before push: every pre-existing byte identical (`cmp` after stripping the three inserted dates), 307 rows and 307 distinct tickers per day, zero overlap with existing rows, no truncated rows, each date landing as exactly one contiguous block (78 → 81 blocks).

⚠️ Two traps found while doing it, both worth remembering:

* **`core.autocrlf=true`** — the working tree is CRLF over an LF blob, so a raw `cmp` of the working file against a `git show` blob **always** fails at line 1. Normalize with `tr -d` on the CR before comparing, or you will chase a phantom.
* **`predictions.csv` carries a malformed 614-line region from mid-May** (embedded newlines in a quoted field, around the 5/14–5/21 blocks). It is untouched and unrelated, but naive line-counting on this file is slightly off and date-block walks must key on **field 1**, not raw line characters.

Rows restore as `scored=False`; the scorer matures 8/12 and 8/13 on the next run and 8/24 around 8/31. **8/21 is NOT recoverable** — the river break meant it was never computed at all.

**⑥ ✅ CAUSE AND DETECTOR BOTH SHIPPED — `86947a6` (PR #25).** Two changes to `quant_daily.yml`:

* **Base refresh** — `ref: github.ref_name` on checkout. A branch *name* makes `actions/checkout` resolve the **live tip** instead of the pinned SHA. This closes the "cron created during the morning cycle" window. It does **NOT** close the "morning commits mid-run" window — nothing in checkout can — which is why the guard is not optional.
* **`Predictions ledger guard`** — compares the pushed tip against its own parent on the remote and **fails the run** if the row count dropped. Nothing in this repo prunes that file, so a shrink is always a bug.

🔑 **The guard is `if: always()` and sits AFTER the flat-invariant gate, deliberately.** HON has that gate permanently red, and **a permanently-failing gate cannot carry a new signal** — placing the guard before it would let the next wipe hide behind the same red check that hid this one. That is the 8/22 lesson repeated in a second form. The guard fails **open** (warning, exit 0) on shallow history, a missing file, or a non-master branch, so it can never red a run on its own confusion.

Verified before merge: YAML parse confirmed server-side by dispatching on the branch (accepted, then cancelled in setup — `trade` never started). Guard logic extracted and run against real history — **current master `50087 → 51008` OK exit 0; replay of the actual 8/24 wipe (`46be028`) gave `::error::…307 lost` exit 1**; non-master skip exit 0; unreadable file warn exit 0.

**⑦ ✅ THE 8/18–8/19 RANK-IC "GAP" IS NOT A BUG — IT IS THE SETTLE LAG.** `analyze_rank_ic.py` does not read the scorer at all; it recomputes forward returns from price history. With `HORIZON_DEFAULT=5` and `SETTLED_ONLY=1`, a day enters the series only once its exit bar is a **provably completed** session — a **6-session lag**. The 8/25 run said so itself:

```
[rank-ic] withholding 2026-08-18 (n=276) — its exit bar is the newest bar and is
not settled yet; it enters the series next session. Rows already written never move.
```

That rule was added 8/06 precisely because provisional rows silently restated themselves (7/24 −0.1186 → −0.0389; 7/27 flipped sign). 8/18 enters once 8/26 closes, 8/19 once 8/27 does. 🔑 **The real hole was 4 days, not 7:** 8/12, 8/13, 8/24 (now recovered) and 8/21 (gone).

**⑧ 🔑 THE v1 `confidence` SERIES IS STRUCTURALLY DEAD — ONLY `rank_score` COUNTS.** Chasing why 8/11 is in `rank_ic_v2.csv` but missing from `rank_ic.csv`: on 8/11 `confidence` was **exactly 0.5 for 305 of 307 names**. Filter to the 276 with valid forward returns and the score vector has no variance left, so Spearman is undefined and the day drops. The script measures the disease directly:

| | v1 `confidence` | v2 `rank_score` |
|---|---|---|
| Tied at modal value | **97.4%** | 3.5% |
| Names carrying a distinct value | ~7 of 279 | ~268 of 278 |
| Days with NO name below 0.5 | **27/27** | 0/10 |

The script's own verdict on v1: *"this series is NOT a valid cross-sectional rank-IC… Do not gate a GO/NO-GO on it."* Ties dominate, both decile legs fall out in FILE ORDER, the printed `n` overstates the sample, and `beta_roll` cannot identify. **v1's 8/11 hole needs no fix — v1 is unusable by construction.** Same family as the known 0.10-confidence-clamp SELL artifact.

**⑨ 📊 WHERE S4 ACTUALLY STANDS.** `rank_ic_v2` is at **N=6, window 2026-08-06 → 2026-08-17, mean rank-IC −0.0268** against a gate of **≥ +0.03 with t ≥ 2.0**. The sign is wrong, not just the magnitude. The recovery pays out next run: **N should go 6 → 8** (8/12 and 8/13 are both settled), 8/18 joins on 8/27, 8/24 around 9/2. ⚠️ `rank_score` only starts 8/06, so the entire S4 window is ~35 trading days minus the 6-session lag ≈ **~30 observations by the ~9/24 terminal read** — a thin sample for t ≥ 2.0, which makes each recovered day worth proportionally more.

**⑩ 🔴 HON — UNCHANGED, STILL A TRADE THE USER FIRES.** `−13 @ $125.6546` avg entry, now `$215.41`. Market value **−$2,800.33**, unrealized **−$1,166.82 (−71.43%)** — essentially flat against the ~$2,838 read from the 8/20 dry-run. Still `[close_long] skipped short: HON`, still `QT_MAX_GROSS=0` blocking the cover, still no scheduled run that can clear it. **Zero trades on 8/24 and 8/25** (`0 trades executed | 307 predictions logged` both days) — correct and by design under coast-to-flat, not a fault.

---

## 🗓️ SESSION LEDGER — 2026-08-22 (Saturday): 🔴 **THE MORNING RETRAIN BROKE ON 8/21 AND NOTHING PAGED.** An unpinned `river` upgrade aborted Cell 2, which took `_get_macro_history` with it, which killed `build_features`, which trained **`0/0` models** — the first Frame-1 evidence day lost *inside the S4 measurement window*. ✅ Root-caused, dated to the day, FIXED and PUSHED (`01b2897`, `river>=0.21,<0.26`) — **and the structural fix landed the same session** (`1720008`, PR #24: the Cell 2 import is now `try/except`-guarded, pinned by new validate section 19, 13/13 PASS, green on master post-merge). 🔴 **HON still uncovered — now 54 red trading runs since 8/12, zero successes.**

**① 🔴 THE 8/21 MORNING RUN TRAINED ZERO MODELS AND NOBODY NOTICED.** Run `32487604422` (13:35Z, `Run type: morning (UTC hour=13 day=5)`) printed `MORNING cycle complete -- 2026-08-21 13:41 UTC` — **four minutes**, against ~2h for a real retrain:

```
[WARNING] Cell 2 raised an exception:
  .../river/stream/iter_csv.py line 21, in <module>
    class DictReader(csv.DictReader["FeatureName"]):
TypeError: type 'DictReader' is not subscriptable
[WARNING] Cell 6 raised an exception:
  File "<string>", line 335, in build_features
NameError: name '_get_macro_history' is not defined
  DFS: name 'FEATURE_COLS' is not defined
0/0 trained
  [patch] Ridge ensemble members added: 0/0
0 signals generated
  0 trades executed | 0 predictions logged
```

**② 🔑 ROOT CAUSE — AN UNPINNED *DEAD* DEPENDENCY.** `requirements.txt` carried `river>=0.21` with **no upper cap**. Installed river went **0.25.0 on 8/20 → 0.26.0 on 8/21** (both read straight off the run logs). river 0.26 requires **Python 3.12**; `.github/workflows/quant_daily.yml` pins `python-version: '3.11'`. Four links, every one silent:

1. `river` imports at **Cell 2 line 34** → `TypeError` → **Cell 2 aborts.**
2. `_get_macro_history` is defined **below that import, in the same cell** (directly under `print("River online learning imported OK")`) → **never binds.**
3. Cell 6 `build_features` calls it at its line 335 → `NameError` → **zero features built.**
4. Cell 8 → `0/0 trained`, `FEATURE_COLS` undefined, Ridge members `0/0`, DSR `0/0`.

⚠️ **River was CUT from the blend on 6/14** (see the 6/12-14 ledger). A dependency the model no longer uses took down the entire live feature pipeline. This is the same class as the yfinance cap already in `requirements.txt` ("1.5.1 walked in silently 2026-07-06") — **river was the one remaining uncapped package that is imported at module scope.**

**③ 📅 DATED TO THE DAY — one bad run, not a slow decay.** Morning-run `N/N trained` and cycle time, straight off the logs:

| Date | Morning run | Trained | Cycle time |
|------|------------|---------|-----------|
| 8/13 | `31705664569` | 307/307 | ~2h11m |
| 8/17 | `32035849171` | 306/306 | ~2h23m |
| 8/18 | `32143211272` | 306/306 | ~2h14m |
| 8/19 | `32258925302` | 304/304 | ~2h09m |
| 8/20 | `32375097880` | 304/304 | ~2h11m |
| **8/21** | **`32487604422`** | **0/0** | **4m** |

**④ 🔴 THE COST IS EVIDENCE, NOT MONEY.** **No trading harm** — entries were already off (`[patch] Gross cap: … cap 0.00x -> room $0 = 0 new BUY slots`) and both guards printed clean (`Oversell guard: enforce=True pos_map=1 pos_ok=True`, `Sector cap: … ok=True`). What it did cost:

- **Frame 1 lost a day.** Morning commit `803c4ca` touched **ZERO `data/shadow/` files** — the first time that has happened. `cross_sectional_positions.jsonl` stops at 8/20. ⚠️ Do NOT read `rank_ic.csv` (last row 8/10) or `rank_ic_v2.csv` (8/11) as corroboration — those lag by the maturation window **and** are full-overwritten every morning (see the 8/10 ledger); **the jsonl is the load-bearing check.**
- **Frames 2 and 3 were untouched** — `data/shadow_intraday/` and `data/stat_arb/` both advanced normally on 8/21. The break is specific to the Cell 2 → 6 → 8 main-model path.
- `data/predictions/dsr_scores.json` was **overwritten from 306 real scores to `{}`** (5,486 bytes → 2). Self-heals on the first good retrain.
- Cell 15 wrote `RULE WRITTEN: High RSI Bear … BUY signals wrong 75% of time (2 samples)` off a degenerate base. Also self-healing.

🔑 **Why this outranks the red checkmarks: S4 — the terminal Stage-1 read — is due ~2026-09-24 off Frame-1's `rank_ic_v2.csv`.** Every broken morning is a lost day *inside the final measurement window*. And **nothing pages**, because the only red on the run is the pre-existing HON gate — Discord said exactly what it had said the previous 53 times. 🔴 **A silent zero-retrain is invisible while the gate is red. Treat that as a standing hazard until HON clears.**

**⑤ ✅ FIXED AND PUSHED — `01b2897` on master.** One line, plus a comment block in the same idiom as the yfinance cap:
...
This restores **0.25.0 — the exact version that trained 304/304 on 8/20.** ⚠️ **Deliberately NOT bumping the runner to Python 3.12:** a new interpreter is a dated change to the whole stack, and doing it inside the S4 window would contaminate the terminal read. Cap the dependency; do not chase it.

**Verification, in two stages.** (a) *Cheap, no retrain needed* — pip install and Cell 2 run on **every** cycle, so the next scheduled weekend evening run should show `river-0.25.0` in the install output, print `River online learning imported OK` (that line sits immediately above the `_get_macro_history` def, so seeing it means Cell 2 completed), and carry **no** `Cell 2 raised` / `Cell 6 raised` warnings. (b) *The real proof — Mon 2026-08-24 morning*: `~304/304 trained`, a ~2h cycle rather than 4m, `data/shadow/` files back in the auto-commit, and `dsr_scores.json` repopulated from `{}`.

**⑥ 🔴 HON — UNCHANGED, AND THE COUNT HAS GROWN.** **54 failed trading runs since 8/12, zero successes** (plus 24 cancelled). Gate signature byte-identical on every run through 8/21 21:55Z: `armed=True breach=False short_breach=True last_seen_book=1 longs=0 shorts=1 last_closed=0 errors=0`. Long side still reads clean (`breach=False`, `longs=0`) — PR #23 is doing its job, the red is informative rather than deadlocked. A **second** read-only dry-run on 8/20 21:16Z (`32418542070`) re-confirmed the plan independently: `HON short=-13 openBUY=0 price=218.32 cover=13 cost=$2,838` → *"1 market BUY-to-cover (DAY, fill at next open) … Projected after covers: gross ~$0 (0.00x equity), zero shorts."* ⚠️ Cost keeps drifting down as HON falls: **$3,059 (8/12) → $2,864 (8/20 a.m.) → $2,838 (8/20 p.m.)**. The printed `P&L -$1,205` remains the **distorted-basis artifact — do NOT book it.** **This is a trade: the user fires it, not Claude.**

**⑦ ✅ CLOSED SAME SESSION — the Cell 2 import is now GUARDED (`ef0dbdf`, PR #24, merged `1720008`).** The pin stops *this* dependency; the structure was unchanged, so **any** module-level import failure above `_get_macro_history` reproduced the identical silent zeroing. Fixed with **two `_SRC_REPLACE` anchors** (Cell 2): the import is wrapped in `try/except`, the four names bind to `None` on failure, `_RIVER_OK` is set, and the cell can no longer print `River online learning imported OK` when it did not. 🔑 **Audited first — this was the LAST unguarded river import in the pipeline:** the other two (`CELL_15_PREPATCH` at `quant_runner.py:5094` and the notebook's own inside Cell 15) are **already** inside `try` blocks, and no sklearn name collides with the four. ⚠️ Anchors sit at the **HEAD** of `_SRC_REPLACE` deliberately — validate section 2 asserts anchor uniqueness on `_SRC_REPLACE[-3:]` against Cell 13, so appending would have silently evicted a gross-cap anchor from that window (the same trap the 8/06 `rank_score` anchor note flags). **Not a dated MODEL change** — on any run where the import succeeds the patched source is semantically identical.

**⑦b ✅ PINNED BY VALIDATE — new section 19, 13/13 PASS.** Anchor uniqueness in Cell 2 (1× each), both anchors wired into `_SRC_REPLACE`, kept out of the `[-3:]` window, patched Cell 2 compiles, no bare module-level import left, `try/except` present — plus **the behavioural check that reproduces the 8/21 regression directly:** set `sys.modules['river'] = None`, exec the guard, and assert a `def` placed **BELOW** it is still reached.

```
19. a broken river does NOT raise                        got=True    PASS
19. _RIVER_OK is False                                   got=False   PASS
19. names bound to None, not left unbound                got=[None, None, None, None] PASS
19. execution REACHES defs below the import              got=True    PASS
19. the failure is reported, not swallowed silently      got=True    PASS
```

Validated **on the branch before merge** — Preflight `32604384963` success, Validate Gross-Cap `32604386162` `ALL VALIDATION CHECKS PASSED` — then re-dispatched post-merge on master (`32604523335` / `32604524303`). ⚠️ **This does NOT change the Mon 8/24 verification:** the guard makes a broken import survivable, it does NOT retrain anything. The pin is what restores `~304/304`; item ② of the pickup list still stands.

**⑧ 📌 Housekeeping.** The run URL pasted this session (`runs/25732008349`) is the **5/12 first-ever v25.1 morning run** — it succeeded, and it was already archived on 7/31 to `data/shadow/stale_era_final/archived_runs/`. Nothing new there; see the 7/17 checkpoint row. No stopping-rule bearing: **nothing this session touched alpha, and the search stays closed.** A dependency break is not a "bug found later" that reopens anything — it never produced a read.

## 🗓️ SESSION LEDGER — 2026-08-12 (Wednesday): 🔴 **FIVE CONSECUTIVE RUNS RED — the flat-invariant gate is DEADLOCKED on a HON SHORT that no scheduled run can ever clear.** 🔑 **PROVENANCE RESOLVED (⑦): the 8/11 wind-down SOLD 13 SHARES THE ACCOUNT DID NOT OWN — HON had been flat since 7/20, and that sell IS the short. This RETRACTS 8/11 ledger ②: `open_positions.json` was right, the BROKER POSITION READ was the phantom.** ✅ Fix drafted, validated and **MERGED** (`f46eef9`, PR #23) — ⚠️ **but that does NOT turn master green: HON is STILL NOT COVERED, and only the cover clears the red.**
...
**No order was submitted between 22:19Z on 8/11 and the 15:52Z position read on 8/12**, yet the book went from 0 to a −13 short. 🔑 **RESOLVED SAME SESSION — see ⑦.** The fill audit proves HON was **flat from 7/20** and that the **8/11 wind-down's own `sell x13` created the short**, so the 8/11 evening `book 0` reads were CORRECT and the 8/12 short is genuinely new. The hypothesis in the previous sentence — that HON was already short and the read kept omitting it — is **WRONG**; the omission is real but it is not what happened here. What survives: **8/11's "THE BOOK IS FLAT" was true for the 24 real positions, and false about HON**, which by then was a phantom long that the wind-down converted into a short.
...
**⑦ 🔑 PROVENANCE RESOLVED — AND IT REVERSES THE 8/11 FINDING. THE 8/11 WIND-DOWN SOLD 13 SHARES THE ACCOUNT DID NOT OWN.** Fill audit `31658961505` (read-only, GETs only), using the per-symbol probe added on 8/11 for exactly this question. HON's **complete** order history in the window (ledger span `2026-06-15` → `2026-08-06`, so orders from `2026-06-13`):

```
=== symbol probe: HON ===
  orders in window: 5
  2026-07-07T08:01:37Z  sell qty=9   status=filled     filled_qty=9  @ 230.00
  2026-07-08T03:07:38Z  buy  qty=24  status=canceled   filled_qty=0  @   0.00
  2026-07-09T08:01:32Z  sell qty=3   status=filled     filled_qty=3  @ 222.05
  2026-07-20T15:51:17Z  sell qty=1   status=filled     filled_qty=1  @ 227.11
  2026-08-11T15:48:01Z  sell qty=13  status=filled     filled_qty=13 @ 233.84
  account activities mentioning HON: 2 (of 100 in window)
    2026-08-11  type=FILL  qty=12  side=sell
    2026-08-11  type=FILL  qty=1   side=sell
  activity types in window: {'FILL': 92, 'FEE': 8}
```

**ZERO filled buys** — the only buy was CANCELED. Four filled sells = **26 shares**, against a **−13** position. The arithmetic is forced, no assumption required:

| date | event | position |
|---|---|---|
| 2026-06-13 | window start | **+13** (= −13 + 26) |
| 2026-07-07 | sell 9 | +4 |
| 2026-07-09 | sell 3 | +1 |
| **2026-07-20** | sell 1 | **0 — FLAT** |
| **2026-08-11** | sell 13 | **−13 — NAKED SHORT** |

No corporate action can explain it away: the only activity types in the window are `FILL` and `FEE`, and the only two mentioning HON are the 8/11 sell fills — no SPLIT/MRGR/SPIN/journal touched it.

🔑 **So HON went flat on 7/20 and stayed flat for three weeks, and the 8/11 wind-down's `sell HON x13` IS the short.** This **inverts 8/11 ledger ②** (now retracted in place): `open_positions.json` was **right** that HON was gone; the **broker position read was wrong**, reporting a phantom long 13. That is forced too — `close_long` takes its sell qty straight from `get_all_positions()` (`_qty13 = int(_held13[_cl_tk]) - ...`), so a 13-share sell means that read returned `+13`. The 8/11 corroboration (book 25 vs ledger 24, Industrials **+$3,253** overnight) was **the phantom, not a discovery.**

⚠️ **The position endpoint has now been caught disagreeing with the order history in BOTH directions** — omitting HON on five consecutive 8/10 reads, then inventing it on 8/11. Neither the ledger nor `get_all_positions()` is authoritative on its own; **only the order history is.**

🔴 **LOGGED, NOT ACTIONED — the gap this exposes.** The oversell guard `d3e55fd` caps `execute_trade` SELLs at live broker long qty, but **`close_long` is a separate path that trusts `get_all_positions()` outright.** Nothing cross-checks a sell against order history, so a phantom long becomes a naked short with every guard reporting green — `pos_ok=True` was true and useless on 8/11. The merged fix (⑤) catches the **consequence** loudly but not the cause. Recorded per the standing decision; **not built on.**

✅ **One practical consequence: the cover is known-safe.** It is a clean naked short with no hidden long behind it, so `cover_shorts()` buying exactly 13 lands the account flat.
...
**② 🔻 RETRACTED 2026-08-13 — THIS ITEM'S CONCLUSION IS BACKWARDS. DO NOT QUOTE IT.** The fill audit's HON probe (`31658961505`, 8/13) shows **ZERO filled HON buys in the whole window and four filled sells totalling 26 shares against a −13 position** — so HON was `+13` on 6/13, hit **FLAT on 2026-07-20**, and stayed flat for three weeks. **`open_positions.json` was RIGHT; the BROKER POSITION READ was wrong**, reporting a phantom long 13 that `close_long` then "closed" — and that 8/11 sell of 13 IS the naked short the account now carries. The "book 25 vs ledger 24" and "Industrials +$3,253" evidence below is **the phantom, not a discovery.** See the 8/12 ledger ⑦ for the full reconstruction. The original text is kept verbatim below as the record of what was believed on 8/11:
...
|---|---|-----|---------|-------------|
| ~~**S1**~~ 🛑 **LANDED — FAILED** | **Frame 3 (stat-arb)** first decision-grade read at ≥30 obs | ~~≈Wed 2026-08-12~~ **read 8/26 at N=40** | ann SR **−2.08** on **N=40** (2026-06-30 → 08-26), mean −0.0627%/day, cum −2.52%, net −$2,007, 38% days positive; 11 closed trades, **8 exited `decointegrated` for −$1,944** vs 3 reversions +$1,152. *Prior read 8/12: SR −0.84 on 27.* | annualised SR **< +0.5** → **RETIRE the frame.** Do NOT extend the window, do NOT rebuild the pair selection. ⚠️ **The runner prints `Frame 3 gate: PASS` — that covers DRAWDOWN AND BETA ONLY.** The script marks Sharpe *"report-only — no blind threshold committed"*; **this table is the threshold.** Do not read the script's PASS as clearing S1. |
| ~~**S2**~~ 🛑 **LANDED 2026-08-26 — FAILED** | **Frame 2 (intraday)** GO/NO-GO — instrument audited clean 8/06, so this read is FINAL when it lands | ~~≈Fri 2026-08-21~~ **read 8/26 at N=32** | **N=32**, mean rank-IC **−0.0139**, **t = −1.17**; trailing-20d **−0.0267, t −1.85** (*more* negative — decaying, not converging); mean L/S −0.0009, cum −3.01%, Sharpe −2.06, 38% win days; **beta vs SPY −0.29 → FAIL independently**. *Prior read: n=19, −0.0008, t −0.05.* | rank-IC **t < 1.0** → **RETIRE the frame.** ✅ **TRIGGERED — Frame 2 is RETIRED as of 2026-08-26.** The read was pre-registered FINAL, so there is no instrumentation escape hatch: **do NOT extend the window and do NOT rebuild the intraday feature set.** Frame 2 is measurement-only history from here. See the 8/26 ledger ③. |
...
| **S4** | **Frame 1 v2** at 30 obs — **the only criterion still pending** | **~2026-09-28** | **N=15** (read 2026-09-04 p.m.), window 08-06 → 08-27, mean rank-IC **+0.0398**, **t = +1.28**. ⚠️ To reach t≥2.0 at N=30 the remaining 18 sessions must average **+0.064/day** — five times the full-window mean. *Prior 8/26: N=9, +0.0153, t=0.33.* Sign is now right; magnitude and significance are not. ⚠️ Raw L/S beta vs SPY **−1.27** and residual beta **+2.24** both FAIL, but only 3/9 rows are hedged — still warm-up, **quote the RAW leg**. *Prior read 8/25: N=6, −0.0268 — contaminated by the wipe; two recovered days flipped the sign.* | fails the Stage-1 gate (rank-IC ≥ +0.03 **and** t ≥ 2.0) → **RETIRE Frame 1.** 🔑 The 8/26 improvement is the **expected** consequence of un-erasing three prediction days, not evidence of alpha — a favourable read on N=9 is not a pass, and per the anti-deferral rule it is not grounds to re-litigate anything. Must also clear **WRC/SPA**. |
