...
**④ Handed to Wednesday 7/15:** verify the 7/15 morning run prints BOTH new tags —
`panel=all-days-v2 (baseline 7/14: 0.4973/-0.0130)` on the walkforward summary line
(`3f570b1`) and the rank-ic restart-exclusion line (`b2a15f5`) — and that walkforward.json
carries the `panel`/`baseline` fields; fill_audit ~7/17+; Sat 7/18 cron proof; attn_vol20
trainer-exclusion decision still open.
