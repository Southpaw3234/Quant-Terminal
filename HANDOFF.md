...
- ⚠️ **Check 4 (Alpaca daily-loss brake) STILL fails open** — same `except Exception: pass`, deliberately left alone. Different risk profile: it wraps a **network call**, so transient API hiccups are routine and halting on each would be badly disruptive. The right shape is probably retry-then-halt, or halt only after N consecutive read failures — not a straight copy of ⑧. Decide before real money; it is the drawdown brake.
