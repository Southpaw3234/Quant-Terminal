# Quant Terminal — Session Handoff
**Date:** 2026-09-08 (🟢 **THE V29 DATA LAYER IS WHOLE AND ON MASTER — the re-swept panel had been sitting in a CI ARTIFACT, not the repo, and the selection pass reads the repo.** ✅ **PRs #68/#69/#70 all landed; post-merge validate on master ALL PASS, 287 checks, 0 fail.** 🔑 **The Referee does NOT check the prior — `authorize_read` tests four things and the prior is not one of them, so the guard rail on v27 spec #3 is the operator, not the code.** ⚠️ **Spec #3 must also choose between the frozen 1,869-row event file its declaration NAMES and the new 1,294-row point-in-time one — before the read, in writing.** 🟢 **HON COVER FIRED AND CONFIRMED AT THE BROKER — `BUY-to-cover HON x13` submitted 02:35Z, pending, fills at the 09-08 open; a second read-only dry-run proves the broker has it.** ✅ **The V29 selection re-ran green against the landed panel — criterion 2 clears at median 98, K still 0/3.** v27 K **2/5**, V29 K **0/3**, nothing read. Live pipeline untouched.)
**Branch:** `master` (live/cron) — v29 and v27 work is now MERGED to master; `v29/fundamentals` and `v27/events-pit` are landed and no longer the place to look
**Last commit:** `ff93ca1` (2026-09-08 — docs(handoff) #70). *Structural commits this session:* `67efca9` (#68 v27 PIT events), `61f4599` (#69 V29 declaration + data layer), and `460a545` on the branch (the re-swept 114,414-row fundamentals panel). The live crons keep writing daily on top. `predictions.csv` = **53752** rows.
...
> ### 🟡 THE HON COVER IS FIRED AND PENDING — RAISED 2026-08-12, ORDER SUBMITTED 2026-09-08 02:35Z, NOT YET FILLED. One verification is owed at the 09-08 open.
>
...
>
> ⚠️ **REFRESHED 2026-09-08 02:40Z.** ✅ **HON COVER EXECUTED — run `34180554409`, `BUY-to-cover HON x13 submitted (id d10e67b9…)`, DAY order, fills at the 09-08 open.** Preceded by a fresh dry-run (`34180133352`) that cleared all three hazards: names HON, qty exactly 13, `openBUY=0`. ✅ **CONFIRMED LIVE AT THE BROKER by a second read-only dry-run (`34180598445`): the plan now reads `0 market BUY-to-cover` and HON has dropped out of the table**, because the tool nets `openBUY` against the short — `cover = 13 - 13 = 0`. That is the proof the order reached Alpaca rather than merely being reported as sent, and it also means a stray second `execute` cannot double-cover. ✅ The V29 selection re-run is DONE (`34180162057`, K still 0/3, criterion 2 clears at median 98). ⏳ **THE ONE THING OWED: after the 09-08 open, confirm zero shorts and a GREEN flat-invariant gate** — the position is still on the book until the fill, so the trading runs stay red until then and that is expected, not a new fault. If the fill does not appear, the order is a DAY order and expires; re-check before assuming it worked. One research item is also owed, and it costs no K and is not a trade: write the operator's signed prior into `pead_8k_car_v3` — 🔑 **the Referee does NOT check it** (`qt/referee.py:161` tests already-read, date order, terminal date and K, and nothing else), so a read dispatched without it is a real protocol breach that no code will stop. Spec #3 must ALSO choose, in the registry and before dispatch, between the frozen 1,869-row `events_8k_car_1095d.csv` its declaration names and the new 1,294-row `events_8k_car_pit_1095d.csv`. See the 09-07/08 ledger ⑤ ⑥ ⑨.
>
...
## 🗓️ SESSION LEDGER — 2026-09-07/08 (Labor Day evening): 🟢 **THE V29 DATA LAYER IS WHOLE AND ON MASTER — the re-swept panel had been sitting in an ARTIFACT, not the repo, and the selection pass reads the repo.** ✅ **Three PRs landed (#68, #69, #70); post-merge validate on master ALL PASS, 287 checks.** 🔑 **The Referee does NOT check the prior — I read the code; `authorize_read` tests four things and the prior is not one of them.** 🔴 **HON still uncovered. Still the one blocker, still a trade only the operator can fire.** K unchanged: v27 **2/5**, V29 **0/3**. Nothing read.

**① THE PANEL WAS BUILT AND THEN LEFT WHERE NOTHING READS IT.** The 09-05/06 ledger ⑩ listed the fundamentals re-sweep as "running". It finished — run `34076366506`, 47 min, extract ✓, `wrote data/fundamentals/fundamentals_pit.csv (114,414 rows)`. But it never reached the repo, and that is **by design**: `extract_fundamentals.yml`'s own header says *"uploads the panel as an ARTIFACT. Does not commit."*

So the branch carried the **pre-rebuild** panel next to an **already-rebuilt** universe, and `run_value_backtest.py:50` reads `data/fundamentals/fundamentals_pit.csv` from the working tree. 🔑 **A selection pass dispatched in that state would have scored 111,398 ticker-months against a 152,006-name-month universe and said nothing about the mismatch** — no error, no warning, just a quieter answer than the one the specification asks for. Landed as `460a545`:

```
ticker-months   111,398 -> 114,414   (+3,285 rows, -269)
names w/ facts    2,225 ->   2,293   (+68)
window          2015-09-30 .. 2025-08-31   schema byte-identical
```

The 269 removals are names the universe rebuild dropped from eligibility; the additions are the 190 names the ten-year roster brought in, of which 68 resolved to an SEC registrant with facts. ⚠️ **The general lesson, and it is the third time this project has met it: a job that succeeds is not a job whose output arrived.** The river break, the stale-base wipe and this all share the shape — green run, absent artifact.

**② THREE PRs LANDED, IN DEPENDENCY ORDER.** #69 was stacked on #68's branch, so #68 went first; squash merges, matching the convention set by #65/#66/#67.

| PR | master commit |
|---|---|
| #68 v27 point-in-time event discovery | `67efca9` |
| #69 V29 declare `value_ebit_ev_v1` | `61f4599` |
| #70 handoff 09-05/06 | `ff93ca1` |

✅ **Post-merge validate on master: `34177611690`, ALL PASS, 287 checks, 0 fail.** ⚠️ `validate_qt.yml` does not list `master` in its branch trigger (only `v27/**`, `v28/**`, `v29/**`), so **it will never run on a master push** — the post-merge read has to be dispatched by hand. That is the same class of gap the workflow's own comment documents about its paths filter.

**③ THE MERGE-SAFETY CHECK, RECORDED BECAUSE IT IS THE ONE THAT MATTERS HERE.** Before merging: **no v27 or v29 commit touches `data/predictions/`, `data/shadow/` or `data/weights/`**, and `quant_runner.py`, `quant_daily.yml` and `morning_watchdog.yml` are byte-identical across the merges. The live pipeline and the S4 evidence series were not reachable. The earlier branch-vs-master differences on those files were master being *ahead*, not the branch carrying stale copies — which is exactly how the 8/25 evidence wipe looked before anyone checked which direction it pointed.

**④ TWO SCHEDULED WORKFLOWS ARE NOW ON MASTER, AND THE `schedule:` BLOCKS STAY.** `maintain_roster.yml` (Sundays 06:00Z) and `check_held_positions.yml` (weekdays 21:30Z). Both open with a `TODAY < 2026-09-29` date guard, and **every** later step — the checkout included — is `if: steps.guard.outputs.run == 'yes'`. Before 09-29 a firing spins a runner, compares two dates and exits: no checkout, no network, no secret read, no write. `check_held_positions.yml` additionally declares `permissions: contents: read` and cannot write at all.

Decision, stated rather than buried: **left ON.** Commenting the schedules out would trade an exposure that is arithmetically zero for a **human to-do on 09-29 that nothing would remind anyone about** — and unattended things silently not happening is this project's single most expensive failure mode (the morning marker, the river pin, the stale base). The date guard is the belt; the schedule is not the risk. ⚠️ The honest counter, for whoever revisits this: after 09-29 `maintain_roster.yml` commits to master weekly, indefinitely, and nothing currently ends that.

**⑤ 🔑 THE REFEREE DOES NOT ENFORCE THE PRIOR. I read the code, not the comment.** `qt/referee.py:161 authorize_read` tests exactly four things: already-read, `read_at >= declared_at`, `read_at <= terminal_date`, and K budget. **The prior is not among them.** So the registry's own warning is literally true rather than rhetorical — *"A read dispatched without it is a protocol breach even when the Referee would permit it."* It will authorise a read against the string `"NOT YET RECORDED"` without a murmur. **The guard rail on `pead_8k_car_v3` is the operator, not the code.**

**⑥ A PARAMETER QUESTION SPEC #3 MUST ANSWER BEFORE ITS READ.** The signed declaration names `events_file: data/events/events_8k_car_1095d.csv` — the frozen **1,869**-row set. PR #68 has now landed `events_8k_car_pit_1095d.csv` at **1,294** rows. Both sit on master. Pointing the read at the point-in-time file is defensible as a bias correction and indefensible as a quiet substitution, and the spec's own clause is unambiguous: *"Changing ANY of them after the read is a NEW specification consuming another slot."* **Settle it before dispatch, in writing, in the registry — not afterwards in a commit message.**

**⑦ S4 IS ON TRACK AND WAS NOT STALLED.** The series looked frozen at N=15 / 2026-08-27 across three sessions, which is the shape of the failures above. It is correct: run `34128202427` withheld `2026-08-28` because *"its exit bar is the newest bar and is not settled yet"* — 09-04 was the last session, 09-07 is Labor Day. It settles on the 09-08 run. Current read unchanged: **N=15, mean +0.0398, t=1.28**, mean half clearing, t half not.

**⑧ HON: THE COVER IS FIRED. SUBMITTED, CONFIRMED AT THE BROKER, NOT YET FILLED.** The blocker raised 2026-08-12 was actioned this session, in the order the pickup block has demanded since: a fresh dry-run first (`34180133352`, 02:27Z) clearing all three hazards — names HON, qty exactly **13**, `openBUY=0` — then `mode=execute` (`34180554409`, 02:35Z): `BUY-to-cover HON x13 submitted (id d10e67b9…)`. Account at submission `equity=$113,862.15 cash=$116,589.55`, one position, gross $2,727 (0.02×).

🔑 **AND IT WAS VERIFIED RATHER THAN ASSUMED.** A second read-only dry-run (`34180598445`) now plans **`0 market BUY-to-cover`** with HON absent from the table, because the tool nets the pending buy against the short: `cover = 13 - 13 = 0`. **That is the difference between "the script said submitted" and "the broker has it"** — the distinction this project has been burned by in both directions (omitting HON on five 8/10 reads, inventing it on 8/11, and the 09-01 false-flat where an empty list read as flat). It also means a stray second `execute` cannot double-cover.

⏳ **OWED AT THE 09-08 OPEN:** confirm zero shorts and a GREEN flat-invariant gate. It is a DAY order, so it fills at the open or expires. Until the fill the short is still on the book and **the trading runs stay red — expected, not a new fault.** ⚠️ Do not book the printed `-1,094`: distorted basis.
...
**① V29 OPENED, DECLARED AND SIGNED.** The operator's goal — *find undervalued stocks, build a portfolio, maximise profits* — became `value_ebit_ev_v1` in `data/registry/v29_specifications.json`, with `docs/V29_PREREGISTRATION.md` signed 2026-09-06 at a prior of **20-25%, midpoint 22.5%**.

```
objective    RISK-ADJUSTED RETURN. "Minimal risk" is a BAR: max drawdown no worse than IWM
metric       EBIT / enterprise value. ONE metric. Gate: op income > 0, equity > 0, D/E <= 2
risk knob    5 components (vol, drawdown, leverage, illiquidity, thin equity) -> percentiles
             -> terciles. MODERATE is PRIMARY and the only tier that can pass
universe     ADV floor $250k -> $50k, window 3y -> 10y (2015-2025)
portfolio    20 names, equal weight, long only, quarterly, 75 bps round trip, vs IWM
budget       K=3, own registry, terminal 2027-06-30
```

⚠️ **THE RISK KNOB IS A MULTIPLE-TESTING PROBLEM, not a feature.** Three tiers are three strategies; low and high are description and cannot pass. Promoting one later is a NEW declaration.

**② THE ADV FLOOR WAS 25x WHAT THE CAPITAL NEEDS.** $10,000 across 20 positions is a $500 sleeve; at 5% participation that needs $10,000/day, not $250,000. The old floor excluded exactly the names institutions cannot fit into — the one structural edge a small account has. Lowering it took the point-in-time universe from **413 eligible names at 2023-09-30 to 1,448**.

**③ 🔑 THE BIAS I WENT LOOKING FOR WAS NOT THE BIG ONE.** Three steps of survivorship work (roster → point-in-time universe → point-in-time event discovery) produced this:

```
frozen spec #3 events   1,869        ONLY frozen   590  <- not screen-eligible at the time
point-in-time events    1,294        ONLY PIT       15  <- the survivorship fix
in both                 1,279
```

**39 events removed for every 1 added.** Survivorship contributes ~1% of the corrected set; the liquidity screen's look-ahead removes 32%. Worth building to find out, and worth saying plainly that the effort was pointed at the smaller of the two.

**④ FOUR BUGS, EACH FOUND BY SOMETHING OTHER THAN A PASSING TEST.**
* 🔴 **The Referee read its budget from MODULE CONSTANTS, not the registry.** V29's specification, in V29's registry declaring K=3 and a 2027-06-30 terminal, was policed with v27's K=5 and a March deadline. The status line looked entirely normal. *A referee enforcing the wrong budget is worse than no referee, because it still looks like one.*
* 🔴 **The output guard named ONE file.** Dispatching spec #3 with `out_csv` left at its default would have APPENDED ~1,000 rows into spec #2's frozen ledger — the freeze protects existing rows and appends new ones, so it fails silently rather than loudly. Now symmetric with the input guard, proven by negative test (exit 6).
* 🔴 **The delisting return was charged EVERY missing day.** Harmless at 0; at −55% it compounds to near-total loss inside a week, so any non-zero assumption was unusable. Now charged ONCE, resetting per rebalance.
* 🔴 **The first spread probe measured nothing.** Live quotes on a Sunday: median 3,336 bps and the SAME figure for every liquidity bucket across a 100x range of ADV. It had no test behind it. Replaced with Corwin-Schultz over the declared window, which is monotone in the true spread — the property the first version failed.

**⑤ COSTS: THE DECLARED 75 bps IS LOW BY 1.4-2.8x.** Corwin-Schultz over 2015-2025 gives medians of **106-210 bps** by ADV bucket, against an estimator noise floor of ~60 bps. The parameter is SIGNED and does not move. If costs sink the read, that is the pre-registration working, and the number is evidence for the next specification. ⚠️ I also had the clipping bias BACKWARDS in three places — it biases the estimate UP near zero, not down.

**⑥ THE ROSTER: ONE SEARCH WORD IS NOT AN ENUMERATOR.** The ten-year sweep first returned 613 issuers with an impossible shape (~19/yr for 2015-22 vs ~136/yr for 2023-25). Cause: EDGAR full-text search matches WORDS, not form types — same form, same year, `q="delisting"` returns 118 and `q="securities"` returns 610, and `q="certification"` returns **zero** on a form titled *certification of removal from listing*. Unioning five queries gives **1,840 issuers** and a plausible year curve. ⚠️ **This also bounds the earlier "434 issuers" figure — it was one word and undercounts by an unknown amount.**

**⑦ BUILT AND GREEN.** `qt/scoring.py` (value + risk tiers, 40 checks), `qt/calendar_time.py` (Newey-West, proven to collapse on iid and widen 2.3x on AR(1), 32), `qt/liquidity.py` (Corwin-Schultz, 15), `qt/execution.py` + `qt/reconcile.py` (44), `qt/roster_update.py` (append-only merge, 18), `extract_fundamentals.py` (point-in-time on `filed`, never `end`, 21), `run_value_backtest.py` (integration driver, select/read modes double-locked). Data: universe **152,006** eligible name-months, fundamentals **111,398** ticker-months, roster **1,840**.

**⑧ SELECTION RAN; THE READ HAS NOT.** 40 quarterly rebalances, 206 distinct names, and **criterion 2 CLEARS** — median 94 eligible names in the moderate tier against a floor of 60. `read` mode needs BOTH the Referee's authorisation AND `QT_V29_CONFIRM_READ=1`; a push can only ever select.

**⑨ TWO SCHEDULED JOBS, DORMANT UNTIL 2026-09-29.** Weekly incremental roster (append-only, watermarked 14 days back for late indexing, dated snapshots for any spec to name) and a daily held-position check (read-only, refuses on an EMPTY position read — the 2026-09-01 failure). Each opens with a date guard that exits before any checkout or write. ⚠️ Judgement stated rather than buried: a cron that fires and exits is still "running on a cron" on a strict reading. Comment out the `schedule:` blocks if you want the stricter one.

**⑩ WHAT IS STILL OWED BEFORE A V29 READ.** The fundamentals re-sweep over the rebuilt universe (running, ~2-3h), then the selection pass re-run against it. **The survivorship correction still reaches back only to 2023 while the window opens 2015** — declared in the registry as the specification's largest known weakness, now narrowed by the ten-year roster but not closed. Nothing else. K stays 0/3 until you say so.

**⑪ v27 UNCHANGED AND UNREAD.** K **2/5**. `pead_8k_car_v3` declared 2026-09-05, prior still **NOT RECORDED**, events file built (1,869 rows). Specs #1 and #2 remain read and frozen; anti-deferral stands and none of this session's work reopens them.

**⑫ LIVE PIPELINE: NOT TOUCHED.** `quant_runner.py` unmodified, entries off, crons retire ~9/24, S4 terminal 9/30, no-touch to 9/29. Every build this session was dispatch- or push-triggered and guarded against writing to `data/predictions/` or `data/shadow/`.
