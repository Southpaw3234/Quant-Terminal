...
| **Fri 2026-07-17 ~12:15 ET** | **Kill-switch era-gate proof (`49a5884`) + stale-era consumer-gates proof (`c653ad4`) + fill_audit re-read** — fill_audit task `qt-fri-0717-fill-audit` already armed 12:45 PM ET | (a) The 7/17 morning log must NOT print `KILL SWITCH: 5 consecutive losses` (the stale-era 7/10 losses are now excluded; window is EMPTY until fresh-era preds mature ~7/21, and `len(scored)==5` is required to trip). If it still trips on consecutive losses → the rewrite anchor didn't apply, check the `[src rewrite]` lines for Cell 13; a VIX/daily-DD/manual-flag halt is a DIFFERENT (legitimate) path — read the reason string before assuming failure. (b) With 8 slots + guard live, expect the REAL first multi-entry morning since 7/8 — entries conformal-sized, gross-capped, oversell-guarded; verify the gross-cap line stays ≤1.00×. (c) `[oversell]` BLOCKED/capped lines remain expected-and-healthy on any SELL day. (d) fill_audit (12:45 ET): split findings pre/post-7/14, fresh-era ledger prices must sit inside real day ranges. **(e) Consumer-gates proof (`c653ad4`, ⚠️ 7th dated MODEL CHANGE effective 7/17):** Cell 15 must print `Not enough scored outcomes yet for diagnosis` (NOT `Overall accuracy: 47.3% across 40579`), ZERO `RULE WRITTEN` lines, no `Kelly W/L ratios loaded` line (empty until fresh era matures ~7/21+ — kelly_qty runs on its default W/L meanwhile), and the `[src rewrite]` lines for the three new era-gate anchors (Cells 13+15). All EXPECTED prints, not regressions. Attribute any signal-mix/size shift 7/17 to this change (stale-era dampeners + W/L ratios removed), NOT to alpha. Stale-era rules/weights archived in `data/shadow/stale_era_final/` (45 ticker dampeners frozen); live jsons reset to defaults; state self-heals via model_cache.pkl (morning saves clean, intraday loads clean). |
