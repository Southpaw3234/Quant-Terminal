...
**⑧ WHAT THE CHECKS MEAN FOR SPEC #2.** Both preconditions in `V27_FORK_DECISION.md` §⑥ clear. Rough power on the 3-year set at 63 bars: 561 → ~420 after dedup (~25% loss observed) → ~390 settled — above the proposal's ~330 estimate. **Every projection still rides on the assumption that the effect accrues linearly with horizon.** If spec #1's `+1.76%` was a right-skewed noise draw, tripling the horizon multiplies noise, not signal, and `t` does not move. Spec #2 (`horizon 63 + lookback 1,095`) remains **drafted and UNDECLARED.** Declaring is free; reading spends one of the four remaining.
