...
- 🔴 **Monday 8/10 carries TWO proofs, both pre-registered below** — the settled-rows stability check (d), and the first live run of the temporal kill switch. Neither is optional.
- **The kill switch fails OPEN on error** (`try/except: pass` around the whole streak block). Untouched by `77a7beb` and out of its scope. Decide separately whether a risk control should swallow its own exceptions — the fix made the block materially more complex (a groupby, two env casts), so the silent-failure surface grew even though every path is validated.
