...
panel.** ADWIN drift flag same run = same composition shift.

**④ Handed to Wednesday 7/15:** walk-forward panel decision (③ — all-days baseline vs
tails-only panel; the 0.55-0.68 gate band was calibrated on the old panel); confirm the
analyzer prints the restart-exclusion line on the 7/15 morning run and exits clean;
fill_audit ~7/17+; Sat 7/18 cron proof; attn_vol20 trainer-exclusion decision still open.
