...
| **Sat 2026-07-11 10:30 ET** | Weekend scoring-run check (task armed) | Scheduled task `qt-sat-0711-scoring-check`: the 14:00Z Saturday cron must run as `evening`, skip Cell 13 (zero orders), no retrain, scorer backfill healthy, marker stays 2026-07-10, no evidence reverts; the 26 queued trim SELLs stay pending for Monday. A missing run = dropped cron tick (benign on weekends — the self-heal gate is weekday-only). |
| **Mon 2026-07-13 12:15 ET** | **7/13 morning-run verify (task armed)** | Scheduled task `qt-mon-0713-verify` (12:15 PM ET, waits for the run, app must be open) reports checks (a)-(g): trim fills → ~0.87× + FIRST ENTRIES since 7/8 · Frame-2 clock first row · attn_vol20 live · AUC under the 4-change window · one retrain + marker line + 3 clocks · ledger qty stable all day · `[conformal]` sizing lines with post-discount qty. |
